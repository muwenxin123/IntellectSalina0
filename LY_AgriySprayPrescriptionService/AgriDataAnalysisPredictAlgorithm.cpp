...
bool AgriDataAnalysisPredictAlgorithm::createFertilityGridGeometryMap(const std::vector<QPointF>& boundPoings,
...
{
	if (boundPoings.size() <= 2 || lons.size() <= 2 || lats.size() <= 2 || lons.size() != lats.size())
	{
		return false;
	}
	...
	for (size_t x = 0; x < cols; ++x) {
		for (size_t y = 0; y < rows; ++y) {
			//const double centerLon = minLon + i * spaceLon;
			//const double centerLat = minLat + i * spaceLat;
	...
	{
		QPointF currentPoint(lons[idx], lats[idx]);
		if (!isPointInPolygon(currentPoint, boundPoings)) {
			continue;
		}
		double tempLon = 0.0;
		double tempLat = 0.0;
		...
		//{
			...
			ptr.lon = lons[idx];
			ptr.lat = lats[idx];
			ptr.fertility = levels[idx];
			//ptr.origin = origins[idx];
			pointMap[gridKey] = ptr; // 更新为实际坐标
		//}
	}
	...
	std::map<QPoint, Poi, QPointFCompare>& pointMap)
{
	...
	for (size_t x = 0; x < cols; ++x)
	{
		for (size_t y = 0; y < rows; ++y)
		{
			...
			double CenLonR, CenLatR;
			double bearingRad = atan(((y * 1.0) + 0.5) / ((x * 1.0) + 0.5)) + angle;
			double Tempdistance = sqrt(((x * 1.0) + 0.5) * ((x * 1.0) + 0.5) + ((y * 1.0) + 0.5) * ((y * 1.0) + 0.5)) * disSpacing;
			//destination()函数内的单位都为弧度制，需要从度°转为弧度
			osgEarth::GeoMath::destination(osg::DegreesToRadians(minLat), osg::DegreesToRadians(minLon),
			...
			double bearingRad1 = atan((y * 1.0)/ (x * 1.0) ) + angle;
			double Tempdistance1 = sqrt((x * 1.0) * (x * 1.0) + (y * 1.0) * (y * 1.0) ) * disSpacing;
			double leftLatR, leftLonR;
			osgEarth::GeoMath::destination(
				osg::DegreesToRadians(minLat),
				osg::DegreesToRadians(minLon),
				bearingRad1, Tempdistance1,
				leftLatR, leftLonR);
			double LeftLon = osg::RadiansToDegrees(leftLonR);

			double bearingRad2 = atan(((y * 1.0) + 1.0) / ((x * 1.0) + 1.0)) + angle;
			double Tempdistance2 = sqrt(((x * 1.0) + 1.0) * ((x * 1.0) + 1.0) + ((y * 1.0) + 1.0) * ((y * 1.0) + 1.0)) * disSpacing;
			double rightLatR, rightLonR;
			osgEarth::GeoMath::destination(
				osg::DegreesToRadians(minLat),
				osg::DegreesToRadians(minLon),
				bearingRad2, Tempdistance2,
				rightLatR, rightLonR);
			double rightLon = osg::RadiansToDegrees(rightLonR);
