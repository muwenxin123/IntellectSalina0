...
void AStar::setNoflyZonePoints(std::vector<std::vector<QPointF>> _NoFlyZonePoints)
{
...
void AStar::defineObstacle(double Height) {
	...
	for (int i = 0; i < m_lonPointNum; ++i)
	{
		for (int j = 0; j < m_latPointNum; ++j)
		{
			...
			QPointF tempPos(lon, lat);
			if (LY_CollaborativeSearchService::Get().m_RoutePlanningDialog->is_point_in_zone(tempPos,
				LY_CollaborativeSearchService::Get().m_uav_dispose_route_dialog->searchZonePoints) == false)
			{
				m_roadmap[i][j]->setObstacle();
			}

			if (!noFlyAreaPointVec.empty())
			{
				for (auto noArea : noFlyAreaPointVec)
				{
					if (LY_CollaborativeSearchService::Get().m_RoutePlanningDialog->is_point_in_zone(tempPos, noArea) == true)
					{
						m_roadmap[i][j]->setObstacle();
						break;
					}
