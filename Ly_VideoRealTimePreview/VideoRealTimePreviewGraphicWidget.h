#ifndef  VIDEOREALTIMEPREVIEWGRAPHICWIDGET_H
#define VIDEOREALTIMEPREVIEWGRAPHICWIDGET_H
...
class GraphicMapEventWatcher : public QWidget
{
public:
	friend class VideoRealTimePreviewGraphicWidget;
	GraphicMapEventWatcher(VideoRealTimePreviewGraphicWidget* pWgt, QWidget* pParent);
	~GraphicMapEventWatcher();

	// 事件过滤器
	bool eventFilter(QObject *object, QEvent *event)override;

private:
	VideoRealTimePreviewGraphicWidget* m_pWgt;
};


class VedioDetailShowWgt;
class RealTimeGraphicsView;
class RealTimeGraphicsScene;
class VideoRealTimePreviewGraphicWidget : public QWidget
{
    Q_OBJECT
public:
	friend class GraphicMapEventWatcher;
	explicit VideoRealTimePreviewGraphicWidget(QWidget *parent = nullptr);
    ~VideoRealTimePreviewGraphicWidget();
...
private:
	RealTimeGraphicsScene*  m_pScene = nullptr;
	RealTimeGraphicsView*  m_pView = nullptr;

	QWidget* m_pParentWidget = nullptr;											// 2维 3维视图
	GraphicMapEventWatcher* m_wathcer = nullptr;							// 观察2三维试图缩放
