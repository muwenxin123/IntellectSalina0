	...
	, modelProcessor(new ModelProcessor(this))
{
    ...
    connect(m_timer, &QTimer::timeout, this, &VideoThread::onGetDataTimeout);
    m_timer->start(3000);
}
...
void VideoThread::setUavID(QString uavID)
{
    CurrUavID = uavID;
}
...
void VideoThread::processFrame(AVFrame *frame)
{
	...
	//{
	//	modelProcessor->process(matImage, m_videoInfo);
	//}
    //////////////////////////////////////////////////////////////////////////
    for (auto tempProcess : m_modelProcessMap)
    {
        ModelProcessor* temp = tempProcess.second;
        if (/*m_ModelVideoInfo.isOpen && */temp && temp->isModelLoaded())
        {
            temp->process(matImage, m_videoInfo);
        }
    }
...
void VideoThread::onGetDataTimeout()
{
    bool isget = LY_AgriVideoPlayer::Get().m_pISelectYoloModelDialog->getModelMapFlag;
    if (isget)
    {
        QReadLocker lk(&m_lock);
        m_YoloModelInfoMap = LY_AgriVideoPlayer::Get().m_pISelectYoloModelDialog->getSelectModelMap();
        LY_AgriVideoPlayer::Get().m_pISelectYoloModelDialog->getModelMapFlag = false;
        startLoadYoloModel();
    }
}

void VideoThread::startLoadYoloModel()
{
    QReadLocker lk(&m_lock);
    std::map<QString, YoloModelInfo> tempMap = m_YoloModelInfoMap;
    for (auto alreadyProcess = m_modelProcessMap.begin(); alreadyProcess != m_modelProcessMap.end(); )
    {
        auto itera = tempMap.find(alreadyProcess->first);
        if (itera == tempMap.end())
        {
            delete alreadyProcess->second;
            alreadyProcess->second = nullptr;
            alreadyProcess = m_modelProcessMap.erase(alreadyProcess);
        }
        else
        {
            ++alreadyProcess;
    ...
    for (auto iterm = tempMap.begin(); iterm != tempMap.end(); iterm++)
    {
		QString tempQString = iterm->first;
		std::string tempName(tempQString.toUtf8().data(), tempQString.toUtf8().length());
		
        YoloModelInfo tempinfo = iterm->second;
        QString cfg = m_yoloPath + tempinfo.model_cfg;
        QString weights = m_yoloPath + tempinfo.model_weights;
		QString names = m_yoloPath + tempinfo.model_names;

		std::string tempName1(cfg.toUtf8().data(), cfg.toUtf8().length());
		std::string tempName2(weights.toUtf8().data(), weights.toUtf8().length());
		std::string tempName3(names.toUtf8().data(), names.toUtf8().length());

        ModelProcessor* modelprocess = new ModelProcessor(this);
        if (modelprocess->loadModel(cfg, weights, names)) {
            qInfo() << "已切换模型:" << weights;
        ...
        m_modelProcessMap[iterm->first] = modelprocess;
    }
