    ...
    for (const QString& key: jsonObject.keys()) {
        if (jsonObject[key].isString()) {
            ...
            if (translateKeys.contains(key)) {
                ...
                QString xlatString = MavlinkApp()->qgcJSONTranslator().translate(translateContext.toUtf8().constData(), locString.toUtf8().constData(), disambiguation.toUtf8().constData());
...
{
// lysd //     for (int i=0; i<polygonArray.count(); i++) {
// lysd //         const QJsonValue& pointValue = polygonArray[i];
// lysd //
// lysd //         QGeoCoordinate pointCoord;
// lysd //         if (!JsonHelper::loadGeoCoordinate(pointValue, false /* altitudeRequired */, pointCoord, errorString, true)) {
// lysd //             list.clearAndDeleteContents();
// lysd //             return false;
// lysd //         }
// lysd //         list.append(new QGCQGeoCoordinate(pointCoord, parent));
// lysd //     }
...
{
// lysd //     for (int i=0; i<list.count(); i++) {
// lysd //         QGeoCoordinate vertex = list.value<QGCQGeoCoordinate*>(i)->coordinate();
// lysd //
// lysd //         QJsonValue jsonValue;
// lysd //         JsonHelper::saveGeoCoordinate(vertex, false /* writeAltitude */, jsonValue);
// lysd //         polygonArray.append(jsonValue);
// lysd //     }
}
