...
void FactMetaData::_setAppSettingsTranslators(void)
{
    ...
    if (!_enumStrings.count() && (type() == valueTypeDouble || type() == valueTypeFloat)) {
        for (size_t i=0; i<sizeof(_rgAppSettingsTranslations)/sizeof(_rgAppSettingsTranslations[0]); i++) {
            ...
            UnitsSettings* settings = QGCApplication::unitsSettings();
            if(!settings){
                continue;
            }
            ...
            case UnitHorizontalDistance:
                settingsUnits = settings->horizontalDistanceUnits();
                break;
            case UnitVerticalDistance:
                settingsUnits = settings->verticalDistanceUnits();
                break;
            case UnitSpeed:
                settingsUnits = settings->speedUnits();
                break;
            case UnitArea:
                settingsUnits = settings->areaUnits();
                break;
            case UnitTemperature:
                settingsUnits = settings->temperatureUnits();
                break;
            case UnitWeight:
                settingsUnits = settings->weightUnits();
                break;
            default:
                break;
            }
...
const FactMetaData::AppSettingsTranslation_s* FactMetaData::_findAppSettingsUnitsTranslation(const QString& rawUnits, UnitTypes type)
{
    for (size_t i=0; i<sizeof(_rgAppSettingsTranslations)/sizeof(_rgAppSettingsTranslations[0]); i++) {
        ...
        uint unitOption = 0;
        auto unitsSettings = QGCApplication::unitsSettings();
        if(!unitsSettings){
            continue;
        }
        switch (type) {
        case UnitHorizontalDistance:
            unitOption = unitsSettings->horizontalDistanceUnits();
            break;
        case UnitVerticalDistance:
            unitOption = unitsSettings->verticalDistanceUnits();
            break;
        case UnitArea:
            unitOption = unitsSettings->areaUnits();
            break;
        case UnitSpeed:
            unitOption = unitsSettings->speedUnits();
            break;
        case UnitTemperature:
            unitOption = unitsSettings->temperatureUnits();
            break;
        case UnitWeight:
            unitOption = unitsSettings->weightUnits();
            break;
        }
