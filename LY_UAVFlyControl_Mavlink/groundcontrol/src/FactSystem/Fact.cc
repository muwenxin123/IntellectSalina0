...
FactValueSliderListModel* Fact::valueSliderModel(void)
{
    if (!_valueSliderModel) {
        // lysd // _valueSliderModel = new FactValueSliderListModel(*this);
    }
...
{
    if(MavlinkApp()) {
        if (!MavlinkApp()->runningUnitTests()) {
            if (vehicleRebootRequired()) {
                QGCApplication::showRebootAppMessage(tr("Change of parameter %1 requires a Vehicle reboot to take effect.").arg(name()));
            } else if (qgcRebootRequired()) {
                QGCApplication::showRebootAppMessage(tr("Change of '%1' value requires restart of to take effect.").arg(shortDescription()));
            }
        }
    }
}
