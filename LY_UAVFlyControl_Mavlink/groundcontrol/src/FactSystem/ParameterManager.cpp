...
ParameterManager::ParameterManager(Vehicle* vehicle)
    ...
    , _logReplay                        (false) // lysd // (!vehicle->vehicleLinkManager()->primaryLink().expired() && vehicle->vehicleLinkManager()->primaryLink().lock()->isLogReplay())
    ...
    , _tryftp                           (vehicle->apmFirmware())
{
    ...
    _mavlink = qgcApp()->mavlinkProtocol();
    ...
    QFileInfo(QSettings().fileName()).dir().mkdir("../data/ParamCache");
}
...
void ParameterManager::_handleParamValue(int componentId, QString parameterName, int parameterCount, int parameterIndex, MAV_PARAM_TYPE mavParamType, QVariant parameterValue)
{
    ...
    if (_waitingReadParamIndexMap[componentId].count()) {
        qCDebug(ParameterManagerVerbose2Log) << _logVehiclePrefix(componentId) << "_waitingReadParamIndexMap:" << _waitingReadParamIndexMap[componentId].size();// << _waitingReadParamIndexMap[componentId];
    }
    if (_waitingReadParamNameMap[componentId].count()) {
        qCDebug(ParameterManagerVerbose2Log) << _logVehiclePrefix(componentId) << "_waitingReadParamNameMap" << _waitingReadParamNameMap[componentId].size();// << _waitingReadParamNameMap[componentId];
    }
    if (_waitingWriteParamNameMap[componentId].count()) {
        qCDebug(ParameterManagerVerbose2Log) << _logVehiclePrefix(componentId) << "_waitingWriteParamNameMap" << _waitingWriteParamNameMap[componentId].size();// << _waitingWriteParamNameMap[componentId];
    }
...
void ParameterManager::refreshAllParameters(uint8_t componentId)
{
    ...
    LinkInterface* weakLink = _vehicle->link();
    if (!weakLink) {
        return;
    }

    if (weakLink->linkConfiguration()->isHighLatency() || _logReplay) {
    ...
    if (_tryftp && (componentId == MAV_COMP_ID_ALL || componentId == MAV_COMP_ID_AUTOPILOT1)) {
        ...
        if (ftpManager->download(MAV_COMP_ID_AUTOPILOT1, "@PARAM/param.pck",
                                 QStandardPaths::writableLocation(QStandardPaths::TempLocation),// lysd // QString("%1").arg("../data/ParameterTempLocation"), // lysd //
...
Fact* ParameterManager::getParameter(int componentId, const QString& paramName)
{
    ...
    if (!_mapCompId2FactMap.contains(componentId) || !_mapCompId2FactMap[componentId].contains(mappedParamName)) {
        qgcApp()->reportMissingParameter(componentId, _vehicle->weaponTitle() + mappedParamName);
        return &_defaultFact;
    }
...
bool ParameterManager::_fillIndexBatchQueue(bool waitingParamTimeout)
{
    ...
    for(int componentId: _waitingReadParamIndexMap.keys()) {
        if (_waitingReadParamIndexMap[componentId].count()) {
            ...
            qCDebug(ParameterManagerVerbose1Log) << _logVehiclePrefix(componentId) << "_waitingReadParamIndexMap" << _waitingReadParamIndexMap[componentId].size() << _waitingReadParamIndexMap[componentId];
        }
        ...
        for(int componentId: _waitingWriteParamNameMap.keys()) {
            for(const QString &paramName: _waitingWriteParamNameMap[componentId].keys()) {
                ...
                if (_waitingWriteParamNameMap[componentId][paramName] <= _maxReadWriteRetry) {
                    ...
                    qgcApp()->showAppMessage(errorMsg, _vehicle->weaponTitle());
                }
            }
        }
    }
...
QDir ParameterManager::parameterCacheDir()
{
    ...
    return spath + QDir::separator() + "../data/ParamCache";
}
...
void ParameterManager::_tryCacheHashLoad(int vehicleId, int componentId, QVariant hash_value)
{
    ...
    if (crc32_value == hash_value.toUInt()) {
        ...
        if (ParameterManagerDebugCacheFailureLog().isDebugEnabled()) {
            ...
            qgcApp()->showAppMessage(tr("Parameter cache CRC match failed"), _vehicle->weaponTitle());
        }
    }
}
...
bool ParameterManager::_parseParamFile(const QString& filename)
{
    ...
    qCDebug(ParameterManagerVerbose2Log) << "_parseParamFile: magic: 0x" << magic; // lysd // Qt::hex << magic;
