...
**/
class LinkInterface : public QObject // QThread
{
    Q_OBJECT
...
public:
    ...
    // lysd // uint8_t mavlinkChannel              (void) const;
    // lysd // bool    mavlinkChannelIsSet         (void) const;
    ...
    void    writeBytesThreadSafe        (unsigned long long weaponId, const char *bytes, int length);
...
signals:
    void bytesReceived      (LinkInterface* link, unsigned long long int weaponId, QByteArray data);
    void bytesSent          (LinkInterface* link, unsigned long long int weaponId, QByteArray data);
    void connected          (void);
    void disconnected       (void);
    void communicationError (const QString& title, const QString& error);
    void _invokeWriteBytes  (unsigned long long weaponId, QByteArray);
...
protected:
    ...
    ///
    // lysd // virtual bool _allocateMavlinkChannel();
    // lysd // virtual void _freeMavlinkChannel    ();

private slots:
    virtual void _writeBytes(unsigned long long int weaponId, const QByteArray) = 0; // Not thread safe if called directly, only writeBytesThreadSafe is thread safe
