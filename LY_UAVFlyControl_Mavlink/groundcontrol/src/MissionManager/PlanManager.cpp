...
PlanManager::PlanManager(Vehicle* vehicle, MAV_MISSION_TYPE planType)
    : _vehicle                  (vehicle)
    , _missionCommandTree       (QGCApplication::missionCommandTree())
    , _planType                 (planType)
...
void PlanManager::writeMissionItems(const QList<MissionItem*>& missionItems)
{
    ...
    bool skipFirstItem = false;// lysd // _planType == MAV_MISSION_TYPE_MISSION && !_vehicle->firmwarePlugin()->sendHomePositionToVehicle();
...
void PlanManager::_writeMissionCount(void)
{
	...
	//emit updateMissionItemTips(QStringLiteral("WriteMissionCount %1 count: %2, retryCount: %3")
	//	.arg(_planTypeString()).arg(_writeMissionItems.count()).arg(_retryCount));
	emit updateMissionItemTips(tr2("写入航点类型 ") + _planTypeString() + tr2(" 总数：") + QString::number(_writeMissionItems.count()) + tr2("，重试次数：") + QString::number(_retryCount));

    if (_vehicle) {
        mavlink_message_t       message;

        mavlink_msg_mission_count_pack_chan(_vehicle->mavlinkSystemId(),
                                            _vehicle->mavlinkComponentId(),
...
void PlanManager::_requestList(void)
{
    ...
    //emit updateMissionItemTips(QStringLiteral("RequestList %1 planType, retryCount: %2").arg(_planTypeString()).arg(_retryCount));
    emit updateMissionItemTips(tr2("请求航线 ") + _planTypeString() + tr2("，重试次数：") + QString::number(_retryCount));
    ...
    if (_vehicle) {
        ...
        mavlink_msg_mission_request_list_pack_chan(_vehicle->mavlinkSystemId(),
                                                   _vehicle->mavlinkComponentId(),
                                                   _vehicle->mavlinkChannel(),
                                                   &message,
                                                   _vehicle->id(),
...
void PlanManager::_readTransactionComplete(void)
{
    ...
    //emit updateMissionItemTips(QStringLiteral("ReadTransactionComplete read sequence complete"));
    emit updateMissionItemTips(tr2("读取航线完成"));

    if (_vehicle) {
        mavlink_message_t       message;

        mavlink_msg_mission_ack_pack_chan(_vehicle->mavlinkSystemId(),
                                          _vehicle->mavlinkComponentId(),
                                          _vehicle->mavlinkChannel(),
                                          &message,
                                          _vehicle->id(),
...
void PlanManager::_requestNextMissionItem(void)
{
    ...
    //emit updateMissionItemTips(QStringLiteral("RequestNextMissionItem %1 sequenceNumber: %2, retry: %3")
    //                           .arg(_planTypeString()).arg(_itemIndicesToRead[0]).arg(_retryCount));
    emit updateMissionItemTips(tr2("请求下一个任务航点 ") + _planTypeString() + tr2(" 序号：") + QString::number(_itemIndicesToRead[0]) + tr2("，重试次数：") + QString::number(_retryCount));
    if (_vehicle) {
        mavlink_message_t       message;

        mavlink_msg_mission_request_int_pack_chan(_vehicle->mavlinkSystemId(),
                                                  _vehicle->mavlinkComponentId(),
                                                  _vehicle->mavlinkChannel(),
                                                  &message,
                                                  _vehicle->id(),
...
void PlanManager::_handleMissionItem(const mavlink_message_t& message)
{
    ...
    //emit updateMissionItemTips(QStringLiteral("HandleMissionItem %1 seq: %2, command: %3, current: %4, ardupilotHomePositionUpdate: %5")
    //                           .arg(_planTypeString()).arg(seq).arg(command).arg(isCurrentItem).arg(ardupilotHomePositionUpdate));
    emit updateMissionItemTips(tr2("处理任务航点 ") + _planTypeString() + tr2("序号：") + QString::number(seq) + tr2("，命令：") + QString::number(command) + tr2("，是否为当前：") + QString::number(isCurrentItem) + tr2("，Home位置是否更新") + QString::number(ardupilotHomePositionUpdate));
    ...
    emit recvCurrentMissionItemIndex(seq);
...
void PlanManager::_handleMissionRequest(const mavlink_message_t& message)
{
    ...
    emit sendCurrentMissionItemIndex(missionRequestSeq);

    //emit updateMissionItemTips(QStringLiteral("_handleMissionRequest %1 sequenceNumber: %2, command: %3").arg(_planTypeString()).arg(missionRequestSeq).arg(item->command()));
    emit updateMissionItemTips(tr2("处理上传任务航点 ") + _planTypeString() + tr2(" 序号：") + QString::number(missionRequestSeq) + tr2("，命令：") + QString::number(item->command()));

    if (_vehicle) {
        mavlink_message_t       messageOut;

        mavlink_msg_mission_item_int_pack_chan(_vehicle->mavlinkSystemId(),
                                               _vehicle->mavlinkComponentId(),
                                               _vehicle->mavlinkChannel(),
                                               &messageOut,
                                               _vehicle->id(),
                                               ...
                                               item->param7(),
                                               _planType);
        _vehicle->sendMessageOnLinkThreadSafe(messageOut);
    }
    _startAckTimeout(AckMissionRequest);
}
...
void PlanManager::_handleMissionAck(const mavlink_message_t& message)
{
    ...
    if (_vehicle->apmFirmware() && missionAck.type == MAV_MISSION_INVALID_SEQUENCE) {
        ...
        qCDebug(PlanManagerLog) << QStringLiteral("_handleMissionAck ArduPilot sending possibly bogus MAV_MISSION_INVALID_SEQUENCE %1").arg(_planTypeString()) << _planType;
        return;
    }
    ...
    case AckGuidedItem:
        ...
        if (missionAck.type == MAV_MISSION_ACCEPTED) {
            ...
            // FIXME: Protocol error
            _sendError(VehicleAckError, tr("Vehicle returned error: %1. %2Vehicle did not accept guided item.").arg(_missionResultToString((MAV_MISSION_RESULT)missionAck.type)).arg(_planTypeString()));
...
{
    qCDebug(PlanManagerLog) << QStringLiteral("Sending error - _planTypeString(%1) errorCode(%2) errorMsg(%3)").arg(_planTypeString()).arg(errorCode).arg(errorMsg);
...
void PlanManager::stopLoadOrWriteVehicleMission()
{
    _disconnectFromMavlink();
    _ackTimeoutTimer->stop();
    _setTransactionInProgress(TransactionNone);
...
void PlanManager::_removeAllWorker(void)
{
    ...
    if (_vehicle) {
        ...
        mavlink_msg_mission_clear_all_pack_chan(_vehicle->mavlinkSystemId(),
                                                _vehicle->mavlinkComponentId(),
                                                _vehicle->mavlinkChannel(),
                                                &message,
                                                _vehicle->id(),
