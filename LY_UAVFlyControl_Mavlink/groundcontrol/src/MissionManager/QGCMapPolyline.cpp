...
{
    // lysd // connect(&_polylineModel, &QmlObjectListModel::dirtyChanged, this, &QGCMapPolyline::_polylineModelDirtyChanged);
    // lysd // connect(&_polylineModel, &QmlObjectListModel::countChanged, this, &QGCMapPolyline::_polylineModelCountChanged);
...
void QGCMapPolyline::clear(void)
{
    ...
    // lysd // _polylineModel.clearAndDeleteContents();
    {
        while( _polylineModel.size() > 0){
            auto* polylineModelItem = _polylineModel.takeAt(0);
            if(polylineModelItem){
                delete polylineModelItem;
            }
        }
    }
...
void QGCMapPolyline::adjustVertex(int vertexIndex, const QGeoCoordinate coordinate)
{
    ...
    emit pathChanged();
    _polylineModel.at(vertexIndex)->setCoordinate(coordinate);
    setDirty(true);
}
...
void QGCMapPolyline::setDirty(bool dirty)
{
    if (_dirty != dirty) {
        _dirty = dirty;
        if (!dirty) {
            // lysd // _polylineModel.setDirty(false);
        }
...
void QGCMapPolyline::setPath(const QList<QGeoCoordinate>& path)
{
    ...
    for (const QGeoCoordinate& coord: path) {
        ...
        _polylineModel.append(new QGCQGeoCoordinate(coord));
    }
...
void QGCMapPolyline::setPath(const QVariantList& path)
{
    ...
    for (int i=0; i<_polylinePath.count(); i++) {
        _polylineModel.append(new QGCQGeoCoordinate(_polylinePath[i].value<QGeoCoordinate>()));
    }
    setDirty(true);
...
void QGCMapPolyline::splitSegment(int vertexIndex)
{
    ...
    if (nextIndex == 0) {
        ...
        _polylineModel.insert(nextIndex, new QGCQGeoCoordinate(newVertex));
...
void QGCMapPolyline::appendVertex(const QGeoCoordinate& coordinate)
{
    ...
    _polylineModel.append(new QGCQGeoCoordinate(coordinate));
    emit pathChanged();
}
...
void QGCMapPolyline::removeVertex(int vertexIndex)
{
    ...
    QGCQGeoCoordinate* coordObj = _polylineModel.takeAt(vertexIndex);
    delete coordObj;
    ...
    _polylinePath.takeAt(vertexIndex);
    emit pathChanged();
}
...
bool QGCMapPolyline::loadKMLFile(const QString& kmlFile)
{
    ...
    // lysd // if (!KMLHelper::loadPolylineFromFile(kmlFile, rgCoords, errorString)) {
    // lysd //     qgcApp()->showAppMessage(errorString);
    // lysd //     return false;
    // lysd // }
...
void QGCMapPolyline::appendVertices(const QList<QGeoCoordinate>& coordinates)
{
    ...
    QList<QGCQGeoCoordinate*> objects;
    for (const QGeoCoordinate& coordinate: coordinates) {
        objects.append(new QGCQGeoCoordinate(coordinate));
        _polylinePath.append(QVariant::fromValue(coordinate));
    }
...
{
    _resetActive = true;
    // lysd // _polylineModel.beginReset();
}

void QGCMapPolyline::endReset(void)
{
    _resetActive = false;
    // lysd // _polylineModel.endReset();
    emit pathChanged();
}
