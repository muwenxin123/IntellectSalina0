...
bool FixedWingLandingComplexItem::scanForItem(QList<VisualMissionItem*>* visualItems, bool flyView, PlanMasterController* masterController)
{
...
void FixedWingLandingComplexItem::_updateFlightPathSegmentsDontCallDirectly(void)
{
    ...
    if (useLoiterToAlt()->rawValue().toBool()) {
        ...
        // lysd // _appendFlightPathSegment(FlightPathSegment::SegmentTypeLand, loiterTangentCoordinate(), amslEntryAlt(), landingCoordinate(),        amslExitAlt());
    } else {
        // lysd // _appendFlightPathSegment(FlightPathSegment::SegmentTypeLand, finalApproachCoordinate(), amslEntryAlt(), landingCoordinate(),        amslExitAlt());
    }
