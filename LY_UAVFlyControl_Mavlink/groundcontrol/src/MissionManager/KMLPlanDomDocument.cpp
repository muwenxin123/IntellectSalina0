...
KMLPlanDomDocument::KMLPlanDomDocument()
    : KMLDomDocument(QStringLiteral("%1 Plan KML").arg(QGCApplication::applicationName()))
{
    _addStyles();
}
...
void KMLPlanDomDocument::_addFlightPath(Vehicle* vehicle, QList<MissionItem*> rgMissionItems)
{
    ...
    for (const MissionItem* item : rgMissionItems) {
        const MissionCommandUIInfo* uiInfo = QGCApplication::missionCommandTree()->getUIInfo(vehicle, QGCMAVLink::VehicleClassGeneric, item->command());
        if (uiInfo) {
...
void KMLPlanDomDocument::_addComplexItems(QList<VisualMissionItem*>* visualItems)
{
    for (int i=0; i<visualItems->count(); i++) {
        ComplexMissionItem* complexItem = qobject_cast<ComplexMissionItem*>(visualItems->at(i));
        if (complexItem) {
...
void KMLPlanDomDocument::addMission(Vehicle* vehicle, QList<VisualMissionItem*>* visualItems, QList<MissionItem*> rgMissionItems)
{
...
void KMLPlanDomDocument::_addStyles(void)
{
    ...
    addTextElement(lineStyleElement, "color", kmlColorString(Qt::GlobalColor::red));
    addTextElement(lineStyleElement, "width", "4");
    styleElement1.appendChild(lineStyleElement);

    QString kmlSurveyColorString = kmlColorString(Qt::GlobalColor::yellow, 0.5 /* opacity */);
