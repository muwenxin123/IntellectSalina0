    ...
    , _stopTakingVideoFact      (settingsGroup, _metaDataMap[stopTakingVideoName])
{
    ...
    if (QGC::fuzzyCompare(_landingDistanceFact.rawValue().toDouble(), _landingDistanceFact.rawDefaultValue().toDouble())) {
        // lysd // Fact* vtolTransitionDistanceFact = QGCApplication::vtolTransitionDistance();
        double vtolTransitionDistance = QGCApplication::vtolTransitionDistance();
        ...
        _landingDistanceFact.metaData()->setRawMin(QGCApplication::vtolTransitionDistanceMin());
    }
...
bool VTOLLandingComplexItem::scanForItem(QList<VisualMissionItem*>* visualItems, bool flyView, PlanMasterController* masterController)
{
...
void VTOLLandingComplexItem::_updateFlightPathSegmentsDontCallDirectly(void)
{
    ...
    // lysd // _flightPathSegments.beginReset();
    // lysd // _flightPathSegments.clearAndDeleteContents();
    if (useLoiterToAlt()->rawValue().toBool()) {
        // lysd // _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, finalApproachCoordinate(), amslEntryAlt(), loiterTangentCoordinate(),  amslEntryAlt()); // Best we can do to simulate loiter circle terrain profile
        // lysd // _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, loiterTangentCoordinate(), amslEntryAlt(), landingCoordinate(),        amslEntryAlt());
    } else {
        // lysd // _appendFlightPathSegment(FlightPathSegment::SegmentTypeGeneric, finalApproachCoordinate(), amslEntryAlt(), landingCoordinate(),        amslEntryAlt());
    }
    // lysd // _appendFlightPathSegment(FlightPathSegment::SegmentTypeLand, landingCoordinate(), amslEntryAlt(), landingCoordinate(), amslExitAlt());
    // lysd // _flightPathSegments.endReset();
