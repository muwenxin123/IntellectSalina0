...
public:
    ...
    /// Stop Load or write the mission items from the vehicle
    void stopLoadOrWriteVehicleMission(void);
    ...
    static const int _ackTimeoutMilliseconds = 4500;// lysd // 1500;
    // When actively retrying to request mission items, use a shorter timeout instead.
    static const int _retryTimeoutMilliseconds = 1000;// lysd // 250;
    static const int _maxRetryCount = 20;// lysd // 5;

signals:
    void updateMissionItemTips      (const QString& tips);
    ...
    void sendCurrentMissionItemIndex(int sendIndex);
    void recvCurrentMissionItemIndex(int recvIndex);
