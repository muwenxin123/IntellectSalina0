...
void MissionManager::writeArduPilotGuidedMissionItem(const QGeoCoordinate& gotoCoord, bool altChangeOnly)
{
    ...
    if (_vehicle) {
        ...
        mavlink_msg_mission_item_encode_chan(_vehicle->mavlinkSystemId(),
...
void MissionManager::generateResumeMission(int resumeIndex)
{
    ...
    for (int i=0; i<_missionItems.count(); i++) {
        ...
        if (item->command() == MAV_CMD_DO_JUMP) {
            qgcApp()->showAppMessage(_vehicle->weaponTitle() + tr("Unable to generate resume mission due to MAV_CMD_DO_JUMP command."));
            return;
        }
    }
    ...
    const MissionCommandUIInfo* uiInfo = QGCApplication::missionCommandTree()->getUIInfo(_vehicle, _vehicle->vehicleClass(), _missionItems[resumeIndex]->command());
    if (!uiInfo || uiInfo->isStandaloneCoordinate() || !uiInfo->specifiesCoordinate()) {
        ...
        while (--resumeIndex > 0) {
            uiInfo = QGCApplication::missionCommandTree()->getUIInfo(_vehicle, _vehicle->vehicleClass(), _missionItems[resumeIndex]->command());
    ...
    for (int i=0; i<_missionItems.count(); i++) {
        ...
        const MissionCommandUIInfo* uiInfo = QGCApplication::missionCommandTree()->getUIInfo(_vehicle, _vehicle->vehicleClass(), oldItem->command());
