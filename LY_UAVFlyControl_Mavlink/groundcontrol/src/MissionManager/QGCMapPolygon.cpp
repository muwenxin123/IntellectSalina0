...
// lysd // #include "ShapeFileHelper.h"
...
{
    // lysd // connect(&_polygonModel, &QmlObjectListModel::dirtyChanged, this, &QGCMapPolygon::_polygonModelDirtyChanged);
    // lysd // connect(&_polygonModel, &QmlObjectListModel::countChanged, this, &QGCMapPolygon::_polygonModelCountChanged);
...
void QGCMapPolygon::clear(void)
{
    ...
    // lysd // _polygonModel.clearAndDeleteContents();
    {
        while( _polygonModel.size() > 0){
            auto* polylineModelItem = _polygonModel.takeAt(0);
...
void QGCMapPolygon::adjustVertex(int vertexIndex, const QGeoCoordinate coordinate)
{
    ...
    _polygonModel.at(vertexIndex)->setCoordinate(coordinate);
    if (!_centerDrag) {
...
void QGCMapPolygon::setDirty(bool dirty)
{
    if (_dirty != dirty) {
        _dirty = dirty;
        if (!dirty) {
            // lysd // _polygonModel.setDirty(false);
        }
...
void QGCMapPolygon::setPath(const QList<QGeoCoordinate>& path)
{
    ...
    for(const QGeoCoordinate& coord: path) {
        ...
        _polygonModel.append(new QGCQGeoCoordinate(coord));
    }
...
void QGCMapPolygon::setPath(const QVariantList& path)
{
    ...
    for (int i=0; i<_polygonPath.count(); i++) {
        _polygonModel.append(new QGCQGeoCoordinate(_polygonPath[i].value<QGeoCoordinate>()));
    }
...
void QGCMapPolygon::splitPolygonSegment(int vertexIndex)
{
    ...
    if (nextIndex == 0) {
        ...
        _polygonModel.insert(nextIndex, new QGCQGeoCoordinate(newVertex));
...
void QGCMapPolygon::appendVertex(const QGeoCoordinate& coordinate)
{
    ...
    _polygonModel.append(new QGCQGeoCoordinate(coordinate));
    emit pathChanged();
}
...
void QGCMapPolygon::removeVertex(int vertexIndex)
{
    ...
    QGCQGeoCoordinate* coordObj = _polygonModel.takeAt(vertexIndex);
    delete coordObj;
...
bool QGCMapPolygon::loadKMLOrSHPFile(const QString& file)
{
    QString errorString;
    QList<QGeoCoordinate> rgCoords;
    // lysd // if (!ShapeFileHelper::loadPolygonFromFile(file, rgCoords, errorString)) {
...
{
    _resetActive = true;
    // lysd // _polygonModel.beginReset();
}

void QGCMapPolygon::endReset(void)
{
    _resetActive = false;
    // lysd // _polygonModel.endReset();
    emit pathChanged();
