...
// lysd // #include "SettingsManager.h"
...
// lysd // struct EnumInfo_s {
// lysd //     const char *    label;
// lysd //     MAV_FRAME       frame;
// lysd // };

static const struct SimpleMissionItem::EnumInfo_s _rgMavFrameInfo[] = {
...
SimpleMissionItem::SimpleMissionItem(PlanMasterController* masterController, bool flyView, bool forLoad)
    ...
    , _commandTree                      (QGCApplication::missionCommandTree())
...
void SimpleMissionItem::_setupMetaData(void)
{
    ...
    if (!_altitudeMetaData) {
        ...
        if(_defaultParamMetaData == nullptr){
...
QList<SimpleMissionItem::EnumInfo_s> SimpleMissionItem::MavFrameInfoList()
{
    QList<SimpleMissionItem::EnumInfo_s> mavFrameInfoList;
    for (size_t i=0; i<sizeof(_rgMavFrameInfo)/sizeof(_rgMavFrameInfo[0]); i++) {
       mavFrameInfoList.append(_rgMavFrameInfo[i]);
    }
    return mavFrameInfoList;
}
...
FactMetaData *SimpleMissionItem::defaultParamMetaData()
{
    if(_defaultParamMetaData == nullptr){
        _defaultParamMetaData = new FactMetaData(FactMetaData::valueTypeDouble);
        _defaultParamMetaData->setDecimalPlaces(7);
    }
    return _defaultParamMetaData;
}
...
{
    // lysd // _nanFacts.clear();

    // lysd // if (!rawEdit()) {
    // lysd //     _ignoreDirtyChangeSignals = true;

    // lysd //     MAV_CMD command;
    // lysd //     if (_homePositionSpecialCase) {
    // lysd //         command = MAV_CMD_NAV_LAST;
    // lysd //     } else {
    // lysd //         command = _missionItem.command();
    // lysd //     }

    // lysd //     Fact*           rgParamFacts[7] =       { &_missionItem._param1Fact, &_missionItem._param2Fact, &_missionItem._param3Fact, &_missionItem._param4Fact, &_missionItem._param5Fact, &_missionItem._param6Fact, &_missionItem._param7Fact };
    // lysd //     FactMetaData*   rgParamMetaData[7] =    { &_param1MetaData, &_param2MetaData, &_param3MetaData, &_param4MetaData, &_param5MetaData, &_param6MetaData, &_param7MetaData };

    // lysd //     const MissionCommandUIInfo* uiInfo = _commandTree->getUIInfo(_controllerVehicle, _previousVTOLMode, command);

    // lysd //     if (uiInfo) {
    // lysd //         for (int i=1; i<=7; i++) {
    // lysd //             bool showUI;
    // lysd //             const MissionCmdParamInfo* paramInfo = uiInfo->getParamInfo(i, showUI);

    // lysd //             if (showUI && paramInfo && paramInfo->nanUnchanged()) {
    // lysd //                 // Show hide Heading field on waypoint based on vehicle yaw to next waypoint setting. This needs to come from the actual vehicle if it exists
    // lysd //                 // and not _controllerVehicle which is always offline.
    // lysd //                 Vehicle* firmwareVehicle = qgcApp()->toolbox()->multiVehicleManager()->activeVehicle();
    // lysd //                 if (!firmwareVehicle) {
    // lysd //                     firmwareVehicle = _controllerVehicle;
    // lysd //                 }

    // lysd //                 Fact*               paramFact =     rgParamFacts[i-1];
    // lysd //                 FactMetaData*       paramMetaData = rgParamMetaData[i-1];

    // lysd //                 paramFact->_setName(paramInfo->label());
    // lysd //                 paramMetaData->setDecimalPlaces(paramInfo->decimalPlaces());
    // lysd //                 paramMetaData->setRawUnits(paramInfo->units());
    // lysd //                 paramFact->setMetaData(paramMetaData);
    // lysd //                 _nanFacts.append(paramFact);
    // lysd //             }
    // lysd //         }
    // lysd //     }

    // lysd //     _ignoreDirtyChangeSignals = false;
    // lysd // }
}
...
void SimpleMissionItem::setDirty(bool dirty)
{
    if (!_homePositionSpecialCase || (_dirty != dirty)) {
        _dirty = dirty;
        if (!dirty) {
            // lysd // _cameraSection->setDirty(false);
...
void SimpleMissionItem::_setDefaultsForCommand(void)
{
    ...
    _amslAltAboveTerrainFact.setRawValue(qQNaN());
    if (specifiesAltitude()) {
        double defaultAlt = QGCApplication::defaultMissionItemAltitude();
...
{
    // lysd // return _cameraSection->available() ? _cameraSection->specifiedGimbalYaw() : missionItem().specifiedGimbalYaw();
    return missionItem().specifiedGimbalYaw();
}

double SimpleMissionItem::specifiedGimbalPitch(void)
{
    // lysd // return _cameraSection->available() ? _cameraSection->specifiedGimbalPitch() : missionItem().specifiedGimbalPitch();
    return missionItem().specifiedGimbalPitch();
}
...
bool SimpleMissionItem::scanForSections(QList<VisualMissionItem *>* visualItems, int scanIndex, PlanMasterController* /*masterController*/)
{
    bool sectionFound = false;

    // lysd // if (_cameraSection->available()) {
    // lysd //     sectionFound |= _cameraSection->scanForSection(visualItems, scanIndex);
    // lysd // }
...
void SimpleMissionItem::_updateOptionalSections(void)
{
    ...
    // lysd // if (_cameraSection) {
    // lysd //     _cameraSection->deleteLater();
    // lysd //     _cameraSection = nullptr;
   // lysd //  }
    if (_speedSection) {
    ...
    // lysd // _cameraSection = new CameraSection(_masterController, this);
    _speedSection = new SpeedSection(_masterController, this);
    if (static_cast<MAV_CMD>(command()) == MAV_CMD_NAV_WAYPOINT) {
        // lysd // _cameraSection->setAvailable(true);
        _speedSection->setAvailable(true);
    }

    // lysd // connect(_cameraSection, &CameraSection::dirtyChanged,                   this, &SimpleMissionItem::_sectionDirtyChanged);
    // lysd // connect(_cameraSection, &CameraSection::itemCountChanged,               this, &SimpleMissionItem::_updateLastSequenceNumber);
    // lysd // connect(_cameraSection, &CameraSection::availableChanged,               this, &SimpleMissionItem::specifiedGimbalYawChanged);
    // lysd // connect(_cameraSection, &CameraSection::availableChanged,               this, &SimpleMissionItem::specifiedGimbalPitchChanged);
    // lysd // connect(_cameraSection, &CameraSection::specifiedGimbalPitchChanged,    this, &SimpleMissionItem::specifiedGimbalPitchChanged);
    // lysd // connect(_cameraSection, &CameraSection::specifiedGimbalYawChanged,      this, &SimpleMissionItem::specifiedGimbalYawChanged);
    ...
    // lysd // emit cameraSectionChanged(_cameraSection);
...
{
    // lysd // return sequenceNumber() + (_cameraSection ? _cameraSection->itemCount() : 0) + (_speedSection ? _speedSection->itemCount() : 0);
    return sequenceNumber() + (0) + (_speedSection ? _speedSection->itemCount() : 0);
}
...
void SimpleMissionItem::appendMissionItems(QList<MissionItem*>& items, QObject* missionItemParent)
{
    ...
    // lysd // _cameraSection->appendSectionItems(items, missionItemParent, seqNum);
...
void SimpleMissionItem::setMissionFlightStatus(MissionController::MissionFlightStatus_t& missionFlightStatus)
{
    ...
    // lysd // if (_cameraSection->available() && !_cameraSection->specifyGimbal()) {
    // lysd //     if (!qIsNaN(missionFlightStatus.gimbalYaw) && !QGC::fuzzyCompare(_cameraSection->gimbalYaw()->rawValue().toDouble(), missionFlightStatus.gimbalYaw)) {
    // lysd //         _cameraSection->gimbalYaw()->setRawValue(missionFlightStatus.gimbalYaw);
    // lysd //     }
    // lysd //     if (!qIsNaN(missionFlightStatus.gimbalPitch) && !QGC::fuzzyCompare(_cameraSection->gimbalPitch()->rawValue().toDouble(), missionFlightStatus.gimbalPitch)) {
    // lysd //         _cameraSection->gimbalPitch()->setRawValue(missionFlightStatus.gimbalPitch);
    // lysd //     }
    // lysd // }
}
