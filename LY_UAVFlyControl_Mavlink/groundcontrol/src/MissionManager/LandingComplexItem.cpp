    ...
    : ComplexMissionItem        (masterController, flyView)
{
    ...
    // lysd // qgcApp()->addCompressedSignal(QMetaMethod::fromSignal(&LandingComplexItem::_updateFlightPathSegmentsSignal));
}
...
void LandingComplexItem::appendMissionItems(QList<MissionItem*>& items, QObject* missionItemParent)
{
    ...
    if (stopTakingPhotos()->rawValue().toBool()) {
        // lysd // CameraSection::appendStopTakingPhotos(items, seqNum, missionItemParent);
    }

    if (stopTakingVideo()->rawValue().toBool()) {
        // lysd // CameraSection::appendStopTakingVideo(items, seqNum, missionItemParent);
    }
...
bool LandingComplexItem::_scanForItem(QList<VisualMissionItem*>* visualItems, bool flyView, PlanMasterController* masterController, IsLandItemFunc isLandItemFunc, CreateItemFunc createItemFunc)
{
    ...
    SimpleMissionItem* item = qobject_cast<SimpleMissionItem*>(visualItems->at(scanIndex--));
    if (!item) {
        return false;
    }
    ...
    item = qobject_cast<SimpleMissionItem*>(visualItems->at(scanIndex));
    if (!item) {
        return false;
    }
    ...
    // lysd // scanIndex -= CameraSection::stopTakingVideoCommandCount();
    // lysd // bool stopTakingVideo = CameraSection::scanStopTakingVideo(visualItems, scanIndex, false /* removeScannedItems */);
    // lysd // if (!stopTakingVideo) {
    // lysd //     scanIndex += CameraSection::stopTakingVideoCommandCount();
    // lysd // }

    // lysd // scanIndex -= CameraSection::stopTakingPhotosCommandCount();
    // lysd // bool stopTakingPhotos = CameraSection::scanStopTakingPhotos(visualItems, scanIndex, false /* removeScannedItems */);
    // lysd // if (!stopTakingPhotos) {
    // lysd //     scanIndex += CameraSection::stopTakingPhotosCommandCount();
    // lysd // }
    bool stopTakingPhotos = false;
    bool stopTakingVideo  = false;
    ...
    int deleteCount = 3;
    // lysd // if (stopTakingPhotos) {
    // lysd //     deleteCount += CameraSection::stopTakingPhotosCommandCount();
    // lysd // }
    // lysd // if (stopTakingVideo) {
    // lysd //     deleteCount += CameraSection::stopTakingVideoCommandCount();
    // lysd // }
    int firstItem = visualItems->count() - deleteCount;
    while (deleteCount--) {
        visualItems->takeAt(firstItem)->deleteLater();
    }
