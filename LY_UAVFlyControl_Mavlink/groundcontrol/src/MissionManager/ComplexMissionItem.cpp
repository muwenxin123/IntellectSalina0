...
ComplexMissionItem::ComplexMissionItem(PlanMasterController* masterController, bool flyView)
    ...
    // lysd // , _toolbox          (qgcApp()->toolbox())
    // lysd // , _settingsManager  (_toolbox->settingsManager())
{
...
{
    // lysd // if (qgcApp()->toolbox()->corePlugin()->options()->surveyBuiltInPresetNames().contains(name)) {
    // lysd //     qgcApp()->showAppMessage(tr("'%1' is a built-in preset which cannot be deleted.").arg(name));
    // lysd //     return;
    // lysd // }
...
void ComplexMissionItem::_savePresetJson(const QString& name, QJsonObject& presetObject)
{
    ...
    settings.beginGroup(presetsSettingsGroup());
    settings.beginGroup(_presetSettingsKey);
    settings.setValue(name, presetObject.toVariantMap());
...
QJsonObject ComplexMissionItem::_loadPresetJson(const QString& name)
{
    ...
    settings.beginGroup(presetsSettingsGroup());
    settings.beginGroup(_presetSettingsKey);
    return QJsonObject::fromVariantMap(settings.value(name).toMap());
}
...
// lysd // void ComplexMissionItem::_appendFlightPathSegment(FlightPathSegment::SegmentType segmentType, const QGeoCoordinate& coord1, double coord1AMSLAlt, const QGeoCoordinate& coord2, double coord2AMSLAlt)
// lysd // {
// lysd //     // lysd // FlightPathSegment* segment = new FlightPathSegment(segmentType, coord1, coord1AMSLAlt, coord2, coord2AMSLAlt, true /* queryTerrainData */, this /* parent */);
// lysd //
// lysd //     // lysd // connect(segment, &FlightPathSegment::terrainCollisionChanged,       this,               &ComplexMissionItem::_segmentTerrainCollisionChanged);
// lysd //     // lysd // connect(segment, &FlightPathSegment::terrainCollisionChanged,       _missionController, &MissionController::recalcTerrainProfile, Qt::QueuedConnection);
// lysd //     // lysd // connect(segment, &FlightPathSegment::amslTerrainHeightsChanged,     _missionController, &MissionController::recalcTerrainProfile, Qt::QueuedConnection);
// lysd //
// lysd //     // lysd // // Signals may have been emitted in contructor so we need to deal with that now since they were missed
// lysd //
// lysd //     // lysd // _flightPathSegments.append(segment);
// lysd //     // lysd // if (segment->terrainCollision()) {
// lysd //     // lysd //     emit _segmentTerrainCollisionChanged(true);
// lysd //     // lysd // }
// lysd //
// lysd //     // lysd // if (segment->amslTerrainHeights().count()) {
// lysd //     // lysd //     _missionController->recalcTerrainProfile();
// lysd //     // lysd // }
// lysd // }
