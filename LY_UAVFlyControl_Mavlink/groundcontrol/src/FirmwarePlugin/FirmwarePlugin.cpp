...
void FirmwarePlugin::adjustOutgoingMavlinkMessageThreadSafe(Vehicle* /*vehicle*/, mavlink_message_t* /*message*/)
{
    // Generic plugin does no message adjustment
}
...
void FirmwarePlugin::setGuidedMode(Vehicle* vehicle, bool guidedMode)
{
    Q_UNUSED(vehicle);
    Q_UNUSED(guidedMode);
    QGCApplication::showAppMessage(vehicle->weaponTitle() + guided_mode_not_supported_by_vehicle);
}
...
void FirmwarePlugin::guidedModeLand(Vehicle* vehicle)
{
    // Not supported by generic vehicle
    Q_UNUSED(vehicle);
    MavlinkApp()->showAppMessage(vehicle->weaponTitle() + guided_mode_not_supported_by_vehicle);
}
...
void FirmwarePlugin::guidedModeGotoLocation(Vehicle* vehicle, const QGeoCoordinate& gotoCoord)
{
    ...
    Q_UNUSED(vehicle);
    Q_UNUSED(gotoCoord);
    QGCApplication::showAppMessage(vehicle->weaponTitle() + QObject::tr("Guided Mode GotoLocation not supported by Vehicle."));
}

void FirmwarePlugin::guidedModeGotoLocationChangeAltitude(Vehicle *vehicle, const QGeoCoordinate &gotoCoord)
{
    Q_UNUSED(vehicle);
    Q_UNUSED(gotoCoord);
    QGCApplication::showAppMessage(vehicle->weaponTitle() + QObject::tr("Guided Mode GotoLocation ChangeAltitude not supported by Vehicle."));
}

void FirmwarePlugin::guidedModeChangeAltitude(Vehicle* vehicle, double, bool pauseVehicle)
{
    // Not supported by generic vehicle
...
void
FirmwarePlugin::guidedModeChangeGroundSpeed(Vehicle* vehicle, double)
{
    // Not supported by generic vehicle
    QGCApplication::showAppMessage(vehicle->weaponTitle() + guided_mode_not_supported_by_vehicle);
}
...
void
FirmwarePlugin::guidedModeChangeEquivalentAirspeed(Vehicle* vehicle, double)
{
    // Not supported by generic vehicle
    QGCApplication::showAppMessage(vehicle->weaponTitle() + guided_mode_not_supported_by_vehicle);
}
...
bool FirmwarePlugin::_armVehicleAndValidate(Vehicle* vehicle)
{
    ...
    const bool excludeAllEvents = vehicle->excludeAllEvents();
    const int armVehicleAndValidateTimes = (vehicle->armVehicleAndValidateTimes() > 0 && vehicle->armVehicleAndValidateTimes() < 10) ? vehicle->flightModeAndValidateTimes() : 10;

    // Wait 1000 msecs for vehicle to arm
    for (int i=0; i<armVehicleAndValidateTimes; i++) {
        if (vehicle->armed()) {
            vehicleArmed = true;
            break;
        }
        ...
        if(excludeAllEvents){
            QGCApplication::processEvents(QEventLoop::AllEvents);
        }else{
...
bool FirmwarePlugin::_setFlightModeAndValidate(Vehicle* vehicle, const QString& flightMode)
{
    ...
    const bool excludeAllEvents = vehicle->excludeAllEvents();
    const int flightModeAndValidateTryTimes = (vehicle->flightModeAndValidateTryTimes() > 0 && vehicle->flightModeAndValidateTryTimes() < 3) ? vehicle->flightModeAndValidateTryTimes() : 3;
    const int flightModeAndValidateTimes    = (vehicle->flightModeAndValidateTimes() >= 0 && vehicle->flightModeAndValidateTimes() <= 13) ? vehicle->flightModeAndValidateTimes() : 13;

    // We try 3 times
    for (int retries=0; retries<flightModeAndValidateTryTimes; retries++) {
        vehicle->setFlightMode(flightMode);

        // Wait for vehicle to return flight mode
        for (int i=0; i<flightModeAndValidateTimes; i++) {
            if (vehicle->flightMode() == flightMode) {
...
{
    return nullptr;// lysd // new QGCCameraManager(vehicle);
}

// lysd // QGCCameraControl* FirmwarePlugin::createCameraControl(const mavlink_camera_information_t *info, Vehicle *vehicle, int compID, QObject* parent)
// lysd // {
// lysd //     return nullptr;// lysd // new QGCCameraControl(info, vehicle, compID, parent);
// lysd // }
...
void FirmwarePlugin::checkIfIsLatestStable(Vehicle* vehicle)
{
    ...
    if (QGCApplication::runningUnitTests()) {
    ...
    // lysd // QGCFileDownload* downloader = new QGCFileDownload(this);
    // lysd // connect(
    // lysd //     downloader,
    // lysd //     &QGCFileDownload::downloadComplete,
    // lysd //     this,
    // lysd //     [vehicle, this](QString remoteFile, QString localFile, QString errorMsg) {
    // lysd //         if (errorMsg.isEmpty()) {
    // lysd //             _versionFileDownloadFinished(remoteFile, localFile, vehicle);
    // lysd //         } else {
    // lysd //             qCDebug(FirmwarePluginLog) << "Failed to download the latest fw version file. Error: " << errorMsg;
    // lysd //         }
    // lysd //         sender()->deleteLater();
    // lysd //     });
    // lysd // downloader->download(versionFile);
}
...
void FirmwarePlugin::_versionFileDownloadFinished(QString& remoteFile, QString& localFile, Vehicle* vehicle)
{
    ...
    if (currType == FIRMWARE_VERSION_TYPE_OFFICIAL && vehicle->versionCompare(version) < 0) {
        ...
        QGCApplication::showAppMessage(vehicle->weaponTitle() + tr("Vehicle is not running latest stable firmware! Running %1, latest stable is %2.").arg(currentVersionNumber, version));
    }
}
...
{
    // lysd // WeakLinkInterfacePtr weakLink = vehicle->vehicleLinkManager()->primaryLink();
    // lysd // if (!weakLink.expired())
    {
        MAVLinkProtocol*        mavlinkProtocol = QGCApplication::mavlinkProtocol();
        mavlink_follow_target_t follow_target   = {};
        // lysd // SharedLinkInterfacePtr  sharedLink      = weakLink.lock();

        follow_target.timestamp =           QGCApplication::msecsSinceBoot();
...
// lysd // Autotune* FirmwarePlugin::createAutotune(Vehicle *vehicle)
// lysd // {
// lysd //     return new Autotune(vehicle);
// lysd // }
