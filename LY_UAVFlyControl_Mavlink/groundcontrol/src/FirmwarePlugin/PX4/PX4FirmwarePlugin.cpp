...
{
    // lysd // qmlRegisterType<PX4SimpleFlightModesController>     ("QGroundControl.Controllers", 1, 0, "PX4SimpleFlightModesController");
    // lysd // qmlRegisterType<AirframeComponentController>        ("QGroundControl.Controllers", 1, 0, "AirframeComponentController");
    // lysd // qmlRegisterType<SensorsComponentController>         ("QGroundControl.Controllers", 1, 0, "SensorsComponentController");
    // lysd // qmlRegisterType<PowerComponentController>           ("QGroundControl.Controllers", 1, 0, "PowerComponentController");
...
QList<MAV_CMD> PX4FirmwarePlugin::supportedMissionCommands(QGCMAVLink::VehicleClass_t vehicleClass)
{
    ...
    if (QGCApplication::useConditionGate()) {
...
void PX4FirmwarePlugin::guidedModeTakeoff(Vehicle* vehicle, double takeoffAltRel)
{
    ...
    if (qIsNaN(vehicleAltitudeAMSL)) {
        MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to takeoff, vehicle position not known."));
        return;
    }
...
void PX4FirmwarePlugin::guidedModeGotoLocation(Vehicle* vehicle, const QGeoCoordinate& gotoCoord)
{
    if (qIsNaN(vehicle->altitudeAMSL()->rawValue().toDouble())) {
        MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to go to location, vehicle position not known."));
        return;
    }
    ...
    if (vehicle->capabilityBits() & MAV_PROTOCOL_CAPABILITY_COMMAND_INT) {
        vehicle->sendMavCommandInt(vehicle->defaultComponentId(),
                                   ...
                                   gotoCoord.latitude(),
                                   gotoCoord.longitude(),
                                   vehicle->altitudeAMSL()->rawValue().toDouble());
    } else {
        vehicle->sendMavCommand(vehicle->defaultComponentId(),
                                MAV_CMD_DO_REPOSITION,
                                true,   // show error is fails
...
void PX4FirmwarePlugin::guidedModeGotoLocationChangeAltitude(Vehicle *vehicle, const QGeoCoordinate &gotoCoord)
{
    if (qIsNaN(vehicle->altitudeAMSL()->rawValue().toDouble())) {
        MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to go to location, vehicle position not known."));
        return;
    ...
    if (vehicle->capabilityBits() & MAV_PROTOCOL_CAPABILITY_COMMAND_INT) {
        ...
        vehicle->sendMavCommand(vehicle->defaultComponentId(),
                                ...
                                static_cast<float>(gotoCoord.latitude()),
                                static_cast<float>(gotoCoord.longitude()),
                                static_cast<float>(gotoCoord.altitude()));
    }
}
...
void PX4FirmwarePlugin::_changeAltAfterPause(void* resultHandlerData, bool pauseSucceeded)
{
    ...
    if (pauseSucceeded) {
        ...
        MavlinkApp()->showAppMessage(pData->vehicle->weaponTitle() + tr("Unable to pause vehicle."));
    }
...
void PX4FirmwarePlugin::guidedModeChangeAltitude(Vehicle* vehicle, double altitudeChange, bool pauseVehicle)
{
    if (!vehicle->homePosition().isValid()) {
        MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to change altitude, home position unknown."));
        return;
    }
    if (qIsNaN(vehicle->homePosition().altitude())) {
        MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to change altitude, home position altitude unknown."));
        return;
    }
...
void PX4FirmwarePlugin::startMission(Vehicle* vehicle)
{
    if (_setFlightModeAndValidate(vehicle, missionFlightMode())) {
        if (!_armVehicleAndValidate(vehicle)) {
            MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to start mission: Vehicle rejected arming."));
            return;
        }
    } else {
        MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to start mission: Vehicle not changing to %1 flight mode.").arg(missionFlightMode()));
    }
}
...
void PX4FirmwarePlugin::_handleAutopilotVersion(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    if (!instanceData->versionNotified) {
        ...
        if (notifyUser) {
            ...
            MavlinkApp()->showAppMessage(vehicle->weaponTitle() + tr("QGroundControl supports PX4 Pro firmware Version %1.%2.%3 and above. You are using a version prior to that which will lead to unpredictable results. Please upgrade your firmware.").arg(supportedMajorVersion).arg(supportedMinorVersion).arg(supportedPatchVersion));
        }
    }
}
