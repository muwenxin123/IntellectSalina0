...
// lysd // #include "APMFlightModesComponentController.h"
// lysd // #include "APMAirframeComponentController.h"
// lysd // #include "APMSensorsComponentController.h"
// lysd // #include "APMFollowComponentController.h"
// lysd // #include "APMSubMotorComponentController.h"
#include "MissionManager.h"
#include "ParameterManager.h"
// lysd // #include "QGCFileDownload.h"
...
// lysd // #include "LinkManager.h"

// lysd // #include <QTcpSocket>
...
{
    // lysd // qmlRegisterType<APMFlightModesComponentController>  ("QGroundControl.Controllers", 1, 0, "APMFlightModesComponentController");
    // lysd // qmlRegisterType<APMAirframeComponentController>     ("QGroundControl.Controllers", 1, 0, "APMAirframeComponentController");
    // lysd // qmlRegisterType<APMSensorsComponentController>      ("QGroundControl.Controllers", 1, 0, "APMSensorsComponentController");
    // lysd // qmlRegisterType<APMFollowComponentController>       ("QGroundControl.Controllers", 1, 0, "APMFollowComponentController");
    // lysd // qmlRegisterType<APMSubMotorComponentController>     ("QGroundControl.Controllers", 1, 0, "APMSubMotorComponentController");
}
...
void APMFirmwarePlugin::_handleIncomingParamValue(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    uint8_t channel = _reencodeMavlinkChannel(vehicle);
...
void APMFirmwarePlugin::_handleOutgoingParamSetThreadSafe(Vehicle* vehicle, mavlink_message_t* message)
{
...
bool APMFirmwarePlugin::_handleIncomingStatusText(Vehicle* vehicle, mavlink_message_t* message)
{
    ...
    if (messageText.contains("Place vehicle") || messageText.contains("Calibration successful")) {
        _adjustCalibrationMessageSeverity(vehicle, message);
        return true;
    }
...
void APMFirmwarePlugin::adjustOutgoingMavlinkMessageThreadSafe(Vehicle* vehicle, mavlink_message_t* message)
{
    switch (message->msgid) {
    case MAVLINK_MSG_ID_PARAM_SET:
        _handleOutgoingParamSetThreadSafe(vehicle, message);
        break;
    }
}
...
void APMFirmwarePlugin::_setInfoSeverity(Vehicle *vehicle, mavlink_message_t* message) const
{
    // Re-Encoding is always done using mavlink 1.0
    uint8_t channel = _reencodeMavlinkChannel(vehicle);
    QMutexLocker reencode_lock{&_reencodeMavlinkChannelMutex()};
...
void APMFirmwarePlugin::_adjustCalibrationMessageSeverity(Vehicle *vehicle, mavlink_message_t* message) const
{
    mavlink_statustext_t statusText;
    mavlink_msg_statustext_decode(message, &statusText);
...
void APMFirmwarePlugin::initializeStreamRates(Vehicle* vehicle)
{
    ...
    if (qgcApp()->apmStartMavlinkStreams())
    {

        // lysd //APMMavlinkStreamRateSettings* streamRates = qgcApp()->toolbox()->settingsManager()->apmMavlinkStreamRateSettings();
        APMMavlinkStreamRateSettings* streamRates = qgcApp()->apmMavlinkStreamRateSettings();
...
void APMFirmwarePlugin::initializeVehicle(Vehicle* vehicle)
{
    ...
    // lysd // if (qgcApp()->toolbox()->settingsManager()->videoSettings()->videoSource()->rawValue() == VideoSettings::videoSource3DRSolo) {
    // lysd //     _soloVideoHandshake();
    // lysd // }
}
...
QList<MAV_CMD> APMFirmwarePlugin::supportedMissionCommands(QGCMAVLink::VehicleClass_t vehicleClass)
{
    ...
    // lysd // if (qgcApp()->toolbox()->settingsManager()->planViewSettings()->useConditionGate()->rawValue().toBool()) {
    // lysd //     supportedCommands.append(MAV_CMD_CONDITION_GATE);
    // lysd // }
...
{
    // lysd // QTcpSocket* socket = new QTcpSocket(this);

    // lysd // QObject::connect(socket, &QAbstractSocket::errorOccurred, this, &APMFirmwarePlugin::_artooSocketError);
    // lysd // socket->connectToHost(_artooIP, _artooVideoHandshakePort);
}

// lysd // void APMFirmwarePlugin::_artooSocketError(QAbstractSocket::SocketError socketError)
// lysd // {
// lysd //     qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Error during Solo video link setup: %1").arg(socketError));
// lysd // }
...
void APMFirmwarePlugin::guidedModeGotoLocation(Vehicle* vehicle, const QGeoCoordinate& gotoCoord)
{
    if (qIsNaN(vehicle->altitudeRelative()->rawValue().toDouble())) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + QStringLiteral("Unable to go to location, vehicle position not known."));
        return;
    }
...
void APMFirmwarePlugin::guidedModeGotoLocationChangeAltitude(Vehicle *vehicle, const QGeoCoordinate &gotoCoord)
{
    if (qIsNaN(vehicle->altitudeRelative()->rawValue().toDouble())) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + QStringLiteral("Unable to go to location, vehicle position not known."));
        return;
    ...
    QGeoCoordinate coordWithAltitude = gotoCoord;
    // lysd // coordWithAltitude.setAltitude(vehicle->altitudeRelative()->rawValue().toDouble());
    vehicle->missionManager()->writeArduPilotGuidedMissionItem(coordWithAltitude, false /* altChangeOnly */);
}
...
{
    if (qIsNaN(vehicle->altitudeRelative()->rawValue().toDouble())) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to change altitude, vehicle altitude not known."));
        return;
    }

    if (pauseVehicle && !_setFlightModeAndValidate(vehicle, pauseFlightMode())) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to pause vehicle."));
        return;
    }
    ...
    if(vehicle) {
        ...
        MAVLinkProtocol* mavlink = qgcApp()->mavlinkProtocol();
        ...
        vehicle->sendMessageOnLinkThreadSafe( msg);
    }
}
...
bool APMFirmwarePlugin::_guidedModeTakeoff(Vehicle* vehicle, double altitudeRel)
{
    if (!vehicle->multiRotor() && !vehicle->vtol()) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Vehicle does not support guided takeoff"));
        return false;
    }

    double vehicleAltitudeAMSL = vehicle->altitudeAMSL()->rawValue().toDouble();
    if (qIsNaN(vehicleAltitudeAMSL)) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to takeoff, vehicle position not known."));
        return false;
    }
    ...
    if (!_setFlightModeAndValidate(vehicle, "Guided")) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to takeoff: Vehicle failed to change to Guided mode."));
        return false;
    }

    if (!_armVehicleAndValidate(vehicle)) {
        qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to takeoff: Vehicle failed to arm."));
        return false;
    }
...
void APMFirmwarePlugin::startMission(Vehicle* vehicle)
{
    if (vehicle->flying()) {
        ...
        if (!_setFlightModeAndValidate(vehicle, "Auto")) {
            qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to start mission: Vehicle failed to change to Auto mode."));
        }
        return;
    }
    ...
    if (!vehicle->armed()) {
        ...
        if (vehicle->fixedWing()) {
            ...
            if (!_setFlightModeAndValidate(vehicle, "Guided")) {
                qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to start mission: Vehicle failed to change to Guided mode."));
                return;
            }
        }

        if (!_armVehicleAndValidate(vehicle)) {
            qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Unable to start mission: Vehicle failed to arm."));
            return;
        }
    }
...
void APMFirmwarePlugin::_sendGCSMotionReport(Vehicle* vehicle, FollowMe::GCSMotionReport& motionReport, uint8_t estimationCapabilities)
{
    if (!vehicle->homePosition().isValid()) {
        ...
        if (!sentOnce) {
            sentOnce = true;
            qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Follow failed: Home position not set."));
        }
        return;
    }
    ...
    if (!(estimationCapabilities & (FollowMe::POS | FollowMe::VEL | FollowMe::HEADING))) {
        ...
        if (!sentOnce) {
            ...
            qgcApp()->showAppMessage(vehicle->weaponTitle() + tr("Follow failed: Ground station cannot provide required position information."));
        }
        return;
    }

    if (vehicle) {
        MAVLinkProtocol*                mavlinkProtocol = qgcApp()->mavlinkProtocol();
        ...
        mavlink_msg_global_position_int_encode_chan(static_cast<uint8_t>(mavlinkProtocol->getSystemId()),
                                                    ...
                                                    &message,
                                                    &globalPositionInt);
        vehicle->sendMessageOnLinkThreadSafe(message);
    }
}

uint8_t APMFirmwarePlugin::_reencodeMavlinkChannel(Vehicle *vehicle)
{
    return vehicle->mavlinkChannel();
    ...
    //
    // lysd // static QMutex _channelMutex{};
    // lysd // _channelMutex.lock();
    // lysd // static uint8_t channel{LinkManager::invalidMavlinkChannel()};
    // lysd // if (LinkManager::invalidMavlinkChannel() == channel) {
    // lysd //     channel = qgcApp()->toolbox()->linkManager()->allocateMavlinkChannel();
    // lysd // }
    // lysd // _channelMutex.unlock();
    // lysd // return channel;
}
