...
void RequestMetaDataTypeStateMachine::_httpDownloadComplete(QString remoteFile, QString localFile, QString errorMsg)
{
    ...
    // lysd // disconnect(qobject_cast<QGCFileDownload*>(sender()), &QGCFileDownload::downloadComplete, this, &RequestMetaDataTypeStateMachine::_httpDownloadComplete);
    if (errorMsg.isEmpty()) {
        if (_currentFileName) {
    ...
    if (_compInfo->available() && !uri.isEmpty()) {
        ...
        if (cachedFile.isEmpty()) {
            ...
            if (_uriIsMAVLinkFTP(uri)) {
                ...
                // lysd // QGCFileDownload* download = new QGCFileDownload(this);
                // lysd // connect(download, &QGCFileDownload::downloadComplete, this, &RequestMetaDataTypeStateMachine::_httpDownloadComplete);
                // lysd // if (download->download(uri)) {
                // lysd //     _downloadStartTime.start();
                // lysd // } else {
                // lysd //     qCWarning(ComponentInformationManagerLog) << "RequestMetaDataTypeStateMachine::_requestFile QGCFileDownload::download returned failure";
                // lysd //     disconnect(download, &QGCFileDownload::downloadComplete, this, &RequestMetaDataTypeStateMachine::_httpDownloadComplete);
                // lysd //     advance();
                // lysd // }
            }
        } else {
