...
public:
    ...
    // lysd // Q_PROPERTY(int                  motorCount                  READ motorCount                                                     CONSTANT)
    ...
    // lysd // Q_PROPERTY(QGCCameraManager*    cameraManager               READ cameraManager                                                  NOTIFY cameraManagerChanged)
    ...
    // lysd // Q_PROPERTY(QGCMapCircle*    orbitMapCircle  READ orbitMapCircle     CONSTANT)
    ...
    // lysd // Q_PROPERTY(ParameterManager*        parameterManager    READ parameterManager   CONSTANT)
    // lysd // Q_PROPERTY(VehicleLinkManager*      vehicleLinkManager  READ vehicleLinkManager CONSTANT)
    // lysd // Q_PROPERTY(VehicleObjectAvoidance*  objectAvoidance     READ objectAvoidance    CONSTANT)
    // lysd // Q_PROPERTY(Autotune*                autotune            READ autotune           CONSTANT)

    // FactGroup object model properties

    // lysd // Q_PROPERTY(Fact* roll               READ roll               CONSTANT)
    // lysd // Q_PROPERTY(Fact* pitch              READ pitch              CONSTANT)
    // lysd // Q_PROPERTY(Fact* heading            READ heading            CONSTANT)
    // lysd // Q_PROPERTY(Fact* rollRate           READ rollRate           CONSTANT)
    // lysd // Q_PROPERTY(Fact* pitchRate          READ pitchRate          CONSTANT)
    // lysd // Q_PROPERTY(Fact* yawRate            READ yawRate            CONSTANT)
    // lysd // Q_PROPERTY(Fact* groundSpeed        READ groundSpeed        CONSTANT)
    // lysd // Q_PROPERTY(Fact* airSpeed           READ airSpeed           CONSTANT)
    // lysd // Q_PROPERTY(Fact* airSpeedSetpoint   READ airSpeedSetpoint   CONSTANT)
    // lysd // Q_PROPERTY(Fact* climbRate          READ climbRate          CONSTANT)
    // lysd // Q_PROPERTY(Fact* altitudeRelative   READ altitudeRelative   CONSTANT)
    // lysd // Q_PROPERTY(Fact* altitudeAMSL       READ altitudeAMSL       CONSTANT)
    // lysd // Q_PROPERTY(Fact* altitudeTuning     READ altitudeTuning     CONSTANT)
    // lysd // Q_PROPERTY(Fact* altitudeTuningSetpoint READ altitudeTuningSetpoint CONSTANT)
    // lysd // Q_PROPERTY(Fact* xTrackError        READ xTrackError        CONSTANT)
    // lysd // Q_PROPERTY(Fact* rangeFinderDist    READ rangeFinderDist    CONSTANT)
    // lysd // Q_PROPERTY(Fact* flightDistance     READ flightDistance     CONSTANT)
    // lysd // Q_PROPERTY(Fact* distanceToHome     READ distanceToHome     CONSTANT)
    // lysd // Q_PROPERTY(Fact* missionItemIndex   READ missionItemIndex   CONSTANT)
    // lysd // Q_PROPERTY(Fact* headingToNextWP    READ headingToNextWP    CONSTANT)
    // lysd // Q_PROPERTY(Fact* headingToHome      READ headingToHome      CONSTANT)
    // lysd // Q_PROPERTY(Fact* distanceToGCS      READ distanceToGCS      CONSTANT)
    // lysd // Q_PROPERTY(Fact* hobbs              READ hobbs              CONSTANT)
    // lysd // Q_PROPERTY(Fact* throttlePct        READ throttlePct        CONSTANT)

    // lysd // Q_PROPERTY(FactGroup*           gps             READ gpsFactGroup               CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           gps2            READ gps2FactGroup              CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           wind            READ windFactGroup              CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           vibration       READ vibrationFactGroup         CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           temperature     READ temperatureFactGroup       CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           clock           READ clockFactGroup             CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           setpoint        READ setpointFactGroup          CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           escStatus       READ escStatusFactGroup         CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           estimatorStatus READ estimatorStatusFactGroup   CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           terrain         READ terrainFactGroup           CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           distanceSensors READ distanceSensorFactGroup    CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           localPosition   READ localPositionFactGroup     CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           localPositionSetpoint READ localPositionSetpointFactGroup CONSTANT)
    // lysd // Q_PROPERTY(FactGroup*           hygrometer      READ hygrometerFactGroup        CONSTANT)
    // lysd // Q_PROPERTY(QmlObjectListModel*  batteries       READ batteries                  CONSTANT)
    // lysd // Q_PROPERTY(Actuators*           actuators       READ actuators                  CONSTANT)
    // lysd // Q_PROPERTY(HealthAndArmingCheckReport* healthAndArmingCheckReport READ healthAndArmingCheckReport CONSTANT)
    ...
    /// Command vehicle to move to specified location (altitude is included and relative)
    Q_INVOKABLE void guidedModeGotoLocationChangeAltitude(const QGeoCoordinate& gotoCoord);
    ...
    Q_INVOKABLE void triggerSimpleCameraFocus(void);
    ...
    double altitudeRelativeNumber   () { return _altitude_relative; }
    ...
    // Property accesors
    const QString weaponTitle() const;
    const QString& weaponName() const { return _weaponName; }
    unsigned long long weaponId() const { return _weaponId; }
    unsigned int color() const { return _color; }
    void setColor(unsigned int color) { _color = color;}
    int vehicleId() const { return id(); }
    int mavlinkSystemId() const { return _mavlink->getSystemId(); }
    int mavlinkComponentId() const { return _mavlink->getComponentId(); }
    uint8_t mavlinkChannel() const { return _mavlinkChannel; }
    ...
    bool sendMessageOnLinkThreadSafe(mavlink_message_t message);
    ...
    /**
     * @brief Send MAV_CMD_DO_SET_SERVO command to set specified servo pwm in the vehicle
     *
     * @param set specified servo pwm
     */
    void setServoPWM(float servoNumber, float PWM);
    void setServoPWMRepeat(float servoNumber, float PWM, float cycleCount, float cycleTime);
    ...
    /**
     * @brief Send MAV_CMD_DO_PARACHUTE command to trigger specified action in the vehicle
     *
     * @param parachuteAction Parachute action to trigger
     */
    void setParachuteAction(PARACHUTE_ACTION parachuteAction);
    ...
    int excludeAllEvents() { return _excludeAllEvents; }
    void setExcludeAllEvents(const int excludeAllEvents) { _excludeAllEvents = excludeAllEvents; }

    int armVehicleAndValidateTimes() { return _armVehicleAndValidateTimes; }
    void setArmVehicleAndValidateTimes(const int armVehicleAndValidateTimes) { _armVehicleAndValidateTimes = armVehicleAndValidateTimes; }

    int flightModeAndValidateTryTimes() { return _flightModeAndValidateTryTimes; }
    void setFlightModeAndValidateTryTimes(const int flightModeAndValidateTryTimes) { _flightModeAndValidateTryTimes = flightModeAndValidateTryTimes; }

    int flightModeAndValidateTimes() { return _flightModeAndValidateTimes; }
    void setFlightModeAndValidateTimes(const int flightModeAndValidateTimes) { _flightModeAndValidateTimes = flightModeAndValidateTimes; }

    QList<QGCQGeoCoordinate>* cameraTriggerPoints () { return &_cameraTriggerPoints; }
    ...
    QList<FactGroup*>* batteries            () { return &_batteryFactGroupListModel; }
    ...
    void setRollRateFact(double rollRateFact);

    void setPitchRateFact(double pitchRateFact);

    void setYawRateFact(double yawRateFact);
    ...
    void SprayEnable(bool enable = false);
...
signals:
    ...
    void flightModeChangedWeaponId      (unsigned long long int weaponId, int id, int componentId, const QString& flightMode);
...
private slots:
    void _mavlinkMessageReceived            (LinkInterface* link, unsigned long long weaponId, mavlink_message_t message);
    ...
    void _mavlinkMessageStatus              (unsigned long long weaponId, int uasId, uint64_t totalSent, uint64_t totalReceived, uint64_t totalLoss, float lossPercent);
...
private:
    ...
    const QString                   _weaponName;
    const unsigned long long int    _weaponId;
    unsigned int                    _color;
    uint8_t                         _mavlinkChannel = MAVLINK_COMM_0;
    ...
    bool                            _excludeAllEvents              = false;
    int                             _armVehicleAndValidateTimes    = -1;
    int                             _flightModeAndValidateTryTimes = -1;
    int                             _flightModeAndValidateTimes    = -1;
    QElapsedTimer                   _flightTimer;
    QTimer                          _flightTimeUpdater;
    QList<QGCQGeoCoordinate>        _cameraTriggerPoints;
    ...
    QList<FactGroup*>               _batteryFactGroupListModel;
