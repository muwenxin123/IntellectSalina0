...
{
    auto* batteries = vehicle->batteries();

    // We maintain the list in order sorted by battery id so the ui shows them sorted.
    for (int i=0; i<batteries->count(); i++) {
        VehicleBatteryFactGroup* group = qobject_cast<VehicleBatteryFactGroup*>(batteries->at(i));
    ...
    VehicleBatteryFactGroup* newBatteryGroup = new VehicleBatteryFactGroup(batteryId, vehicle);
