...
void APMSensorsComponentController::_startLogCalibration(void)
{
    ...
    if (_calTypeInProgress == CalTypeAccel || _calTypeInProgress == CalTypeCompassMot) {
        // lysd // _nextButton->setEnabled(true);
    }
    // lysd // _cancelButton->setEnabled(_calTypeInProgress == CalTypeOnboardCompass);

    connect(qgcApp()->mavlinkProtocol(), &MAVLinkProtocol::messageReceived, this, &APMSensorsComponentController::_mavlinkMessageReceived);
}
...
void APMSensorsComponentController::_startVisualCalibration(void)
{
    ...
    // lysd // _nextButton->setEnabled(false);
...
{
    disconnect(qgcApp()->mavlinkProtocol(), &MAVLinkProtocol::messageReceived, this, &APMSensorsComponentController::_mavlinkMessageReceived);
    // lysd // _vehicle->vehicleLinkManager()->setCommunicationLostEnabled(true);
...
void APMSensorsComponentController::calibrateAccel(bool doSimpleAccelCal)
{
    ...
    // lysd // _vehicle->vehicleLinkManager()->setCommunicationLostEnabled(false);
    _startVisualCalibration();
    // lysd // _cancelButton->setEnabled(false);
    // lysd // _orientationCalAreaHelpText->setProperty("text", tr("Hold still in the current orientation and press Next when ready"));
...
void APMSensorsComponentController::nextClicked(void)
{
    ...
    if (_vehicle) {
        ...
        mavlink_msg_command_ack_pack_chan(QGCApplication::mavlinkProtocol()->getSystemId(),
                                          QGCApplication::mavlinkProtocol()->getComponentId(),
        ...
        _vehicle->sendMessageOnLinkThreadSafe(msg);
...
void APMSensorsComponentController::_handleMagCalProgress(mavlink_message_t& message)
{
    if (_calTypeInProgress == CalTypeOnboardCompass) {
        ...
        // lysd //     _progressBar->setProperty("value", (float)(_rgCompassCalProgress[0] + _rgCompassCalProgress[1] + _rgCompassCalProgress[2]) / 100.0);
        // lysd // }
    }
}
    ...
    if (commandLong.command == MAV_CMD_ACCELCAL_VEHICLE_POS) {
        ...
        case ACCELCAL_VEHICLE_POS_LEFT:
            if (!_orientationCalLeftSideInProgress) {
                ...
                // lysd // _progressBar->setProperty("value", (qreal)(17 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_RIGHT:
            if (!_orientationCalRightSideInProgress) {
                ...
                // lysd // _progressBar->setProperty("value", (qreal)(34 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_NOSEDOWN:
            if (!_orientationCalNoseDownSideInProgress) {
                ...
                // lysd // _progressBar->setProperty("value", (qreal)(51 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_NOSEUP:
            if (!_orientationCalTailDownSideInProgress) {
                ...
                // lysd // _progressBar->setProperty("value", (qreal)(68 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_BACK:
            if (!_orientationCalUpsideDownSideInProgress) {
                ...
                // lysd // _progressBar->setProperty("value", (qreal)(85 / 100.0));
            }
            break;
...
void APMSensorsComponentController::_mavlinkMessageReceived(LinkInterface* link, unsigned long long weaponId, mavlink_message_t message)
{
    Q_UNUSED(link);
