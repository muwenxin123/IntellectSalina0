...
APMAirframeComponentController::APMAirframeComponentController(void)
    ...
    // lysd // , _frameClassModel  (new QmlObjectListModel(this))
{
    _fillFrameClasses();
}
...
void APMAirframeComponentController::_fillFrameClasses()
{
    ...
    if (qobject_cast<ArduCopterFirmwarePlugin*>(fwPlugin)) {
        ...
        for (int i=1; i<_frameClassFact->enumStrings().count(); i++) {
            ...
            // lysd // _frameClassModel->append(new APMFrameClass(frameClassName, true /* copter */, frameClass, _frameTypeFact, _frameClassModel));
        }
    } else if (qobject_cast<ArduRoverFirmwarePlugin*>(fwPlugin)) {
        for (int i=1; i<_frameClassFact->enumStrings().count(); i++) {
            // lysd // QString frameClassName =    _frameClassFact->enumStrings()[i];
            // lysd // int     frameClass =        _frameClassFact->enumValues()[i].toInt();
            // lysd // _frameClassModel->append(new APMFrameClass(frameClassName, false /* copter */, frameClass, _frameTypeFact, _frameClassModel));
        }
    }
}
...
void APMAirframeComponentController::loadParameters(const QString& paramFile)
{
    ...
    // lysd // connect(downloader, &QGCFileDownload::downloadComplete, this, &APMAirframeComponentController::_githubJsonDownloadComplete);
    // lysd // downloader->download(paramFileUrl.arg(paramFile));
}
...
void APMAirframeComponentController::_githubJsonDownloadComplete(QString /*remoteFile*/, QString localFile, QString errorMsg)
{
    if (errorMsg.isEmpty()) {
        ...
        // lysd // QGCFileDownload* downloader = new QGCFileDownload(this);
        // lysd // connect(downloader, &QGCFileDownload::downloadComplete, this, &APMAirframeComponentController::_paramFileDownloadComplete);
        // lysd // downloader->download(json[QLatin1String("download_url")].toString());
    } else {
        qgcApp()->showAppMessage(_vehicle->weaponTitle() + tr("Param file github json download failed: %1").arg(errorMsg));
        qgcApp()->restoreOverrideCursor();
    }
}
...
void APMAirframeComponentController::_paramFileDownloadComplete(QString /*remoteFile*/, QString localFile, QString errorMsg)
{
    if (errorMsg.isEmpty()) {
        ...
        qgcApp()->showAppMessage(_vehicle->weaponTitle() + tr("Param file download failed: %1").arg(errorMsg));
        qgcApp()->restoreOverrideCursor();
    }
}
