...
// lysd // #if !defined(NO_SERIAL_LINK) && !defined(__android__)
// lysd // #include <QSerialPortInfo>
// lysd // #endif
...
void APMAutoPilotPlugin::_checkForBadCubeBlack(void)
{
    ...
    if (paramMgr->parameterExists(-1, paramAcc3) && paramMgr->getParameter(-1, paramAcc3)->rawValue().toInt() == 0 &&
        ...
        qgcApp()->showAppMessage(_vehicle->weaponTitle() + tr("WARNING: The flight board you are using has a critical service bulletin against it which advises against flying. For details see: https://discuss.cubepilot.org/t/sb-0000002-critical-service-bulletin-for-cubes-purchased-between-january-2019-to-present-do-not-fly/406"));
