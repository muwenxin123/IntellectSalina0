...
SensorsComponentController::SensorsComponentController(void)
    :// lysd //  _statusLog                                (nullptr)
    // lysd // , _progressBar                              (nullptr)
    // lysd // , _compassButton                            (nullptr)
    // lysd // , _gyroButton                               (nullptr)
    // lysd // , _accelButton                              (nullptr)
    // lysd // , _airspeedButton                           (nullptr)
    // lysd // , _levelButton                              (nullptr)
    // lysd // , _cancelButton                             (nullptr)
    // lysd // , _setOrientationsButton                    (nullptr)
    // lysd // ,
...
{
    // lysd // WeakLinkInterfacePtr weakLink = _vehicle->vehicleLinkManager()->primaryLink();
    // lysd // if (weakLink.expired()) {
    // lysd //     return false;
    // lysd // } else {
    // lysd //     SharedLinkInterfacePtr sharedLink = weakLink.lock();
    // lysd //     return sharedLink->linkConfiguration()->type() == LinkConfiguration::TypeUdp;
    // lysd // }
     return false;
}
...
void SensorsComponentController::_appendStatusLog(const QString& text)
{
    ...
    // lysd // QVariant returnedValue;
    // lysd // QVariant varText = text;
    // lysd // QMetaObject::invokeMethod(_statusLog,
    // lysd //                           "append",
    // lysd //                           Q_RETURN_ARG(QVariant, returnedValue),
    // lysd //                           Q_ARG(QVariant, varText));
}
...
void SensorsComponentController::_startLogCalibration(void)
{
    ...
    // lysd // _cancelButton->setEnabled(false);
}

void SensorsComponentController::_startVisualCalibration(void)
{
    // lysd // _compassButton->setEnabled(false);
    // lysd // _gyroButton->setEnabled(false);
    // lysd // _accelButton->setEnabled(false);
    // lysd // _airspeedButton->setEnabled(false);
    // lysd // _levelButton->setEnabled(false);
    // lysd // _setOrientationsButton->setEnabled(false);
    // lysd // _cancelButton->setEnabled(true);

    _resetInternalState();

    // lysd // _progressBar->setProperty("value", 0);
}
...
void SensorsComponentController::_stopCalibration(SensorsComponentController::StopCalibrationCode code)
{
   ...
   // lysd // _compassButton->setEnabled(true);
   // lysd // _gyroButton->setEnabled(true);
   // lysd // _accelButton->setEnabled(true);
   // lysd // _airspeedButton->setEnabled(true);
   // lysd // _levelButton->setEnabled(true);
   // lysd // _setOrientationsButton->setEnabled(true);
   // lysd // _cancelButton->setEnabled(false);

   if (code == StopCalibrationSuccess) {
       _resetInternalState();

   // lysd //     _progressBar->setProperty("value", 1);
   } else {
   // lysd //     _progressBar->setProperty("value", 0);
   }
    ...
    switch (code) {
        case StopCalibrationSuccess:
            // lysd // _orientationCalAreaHelpText->setProperty("text", tr("Calibration complete"));
        ...
        default:
            // Assume failed
            _hideAllCalAreas();
            qgcApp()->showAppMessage(_vehicle->weaponTitle() + tr("Calibration failed. Calibration log will be displayed."));
            break;
    }
...
void SensorsComponentController::_handleUASTextMessage(int uasId, int compId, int severity, QString text)
{
    ...
    if (text.contains("progress <")) {
        ...
        // lysd // bool ok;
        // lysd // int p = percent.toInt(&ok);
        // lysd // if (ok) {
            // lysd // if (_progressBar) {
            // lysd //     _progressBar->setProperty("value", (float)(p / 100.0));
            // lysd // } else
            {
    ...
    if (text.startsWith(calStartPrefix)) {
        ...
        if (text == "accel" || text == "mag" || text == "gyro") {
            ...
            // lysd // _orientationCalAreaHelpText->setProperty("text", tr("Place your vehicle into one of the Incomplete orientations shown below and hold it still"));
    ...
    if (text.endsWith("orientation detected")) {
        ...
        if (_magCalInProgress) {
            // lysd // _orientationCalAreaHelpText->setProperty("text", tr("Rotate the vehicle continuously as shown in the diagram until marked as Completed"));
        } else {
            // lysd // _orientationCalAreaHelpText->setProperty("text", tr("Hold still in the current orientation"));
        }
    ...
    if (text.endsWith("side done, rotate to a different side")) {
        ...
        // lysd // _orientationCalAreaHelpText->setProperty("text", tr("Place you vehicle into one of the orientations shown below and hold it still"));
    ...
    if (text.endsWith("side already completed")) {
        // lysd // _orientationCalAreaHelpText->setProperty("text", tr("Orientation already completed, place you vehicle into one of the incomplete orientations shown below and hold it still"));
        return;
    }
...
{
    if (success) {
        qgcApp()->showAppMessage(_vehicle->weaponTitle() + tr("Reset successful"));
        ...
        qgcApp()->showAppMessage(_vehicle->weaponTitle() + tr("Reset failed"));
    }
}
