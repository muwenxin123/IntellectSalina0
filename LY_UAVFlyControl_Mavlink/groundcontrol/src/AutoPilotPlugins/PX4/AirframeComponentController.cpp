    ...
    _showCustomConfigPanel(false)
{
    if (!_typesRegistered) {
        ...
        // lysd // qmlRegisterUncreatableType<AirframeType>("QGroundControl.Controllers", 1, 0, "AiframeType", "Can only reference AirframeType");
        // lysd // qmlRegisterUncreatableType<Airframe>("QGroundControl.Controllers", 1, 0, "Aiframe", "Can only reference Airframe");
    }
...
{
    if (QGCApplication::multiVehicleManager()->vehicles()->count() > 1) {
        QGCApplication::showAppMessage(_vehicle->weaponTitle() + tr("You cannot change airframe configuration while connected to multiple vehicles."));
        return;
    }

    QGCApplication::setOverrideCursor(Qt::WaitCursor);
...
void AirframeComponentController::_rebootAfterStackUnwind(void)
{
    ...
    QGCApplication::restoreOverrideCursor();
    // lysd // qgcApp()->toolbox()->linkManager()->disconnectAll();
}
