...
#include "PX4RadioComponent.h"
#include "Common/ESP8266Component.h"
#include "FlightModesComponent.h"
#include "SensorsComponent.h"
#include "SafetyComponent.h"
#include "CameraComponent.h"
#include "PowerComponent.h"
#include "Common/MotorComponent.h"
#include "PX4TuningComponent.h"
#include "PX4FlightBehavior.h"
#include "Common/SyslinkComponent.h"

PX4AutoPilotPlugin::PX4AutoPilotPlugin(Vehicle* vehicle, QObject* parent)
    : AutoPilotPlugin(vehicle, parent)
    , _incorrectParameterVersion(false)
    , _airframeFacts(nullptr)
    ...
    if (_components.count() == 0 && !_incorrectParameterVersion) {
        if (_vehicle) {
            if (_vehicle->parameterManager()->parametersReady()) {
                ...
                // lysd // if (_vehicle->actuators()) {
                // lysd //     _vehicle->actuators()->init(); // At this point params are loaded, so we can init the actuators
                // lysd // }
                // lysd // if (_vehicle->actuators() && _vehicle->actuators()->showUi()) {
                // lysd //     _actuatorComponent = new ActuatorComponent(_vehicle, this, this);
                // lysd //     _actuatorComponent->setupTriggerSignals();
                // lysd //     _components.append(QVariant::fromValue(static_cast<VehicleComponent*>(_actuatorComponent)));
                // lysd // } else
                {
...
void PX4AutoPilotPlugin::parametersReadyPreChecks(void)
{
    ...
    if (_vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, hitlParam) &&
        ...
        MavlinkApp()->showAppMessage(_vehicle->weaponTitle() + tr("Warning: Hardware In The Loop (HITL) simulation is enabled for this vehicle."));
    }
}
