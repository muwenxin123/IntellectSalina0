...
void RadioComponentController::_setupCurrentState(void)
{
    ...
    // lysd // _statusText->setProperty("text", instructions);
    ...
    // lysd // _nextButton->setEnabled(state->nextFn != nullptr);
    // lysd // _skipButton->setEnabled(state->skipFn != nullptr);
}
    ...
    if (_currentStep == -1) {
        ...
        if (_chanCount < _chanMinimum) {
            if (_unitTestMode) {
                ...
                qgcApp()->showAppMessage(_vehicle->weaponTitle() + QString("Detected %1 radio channels. To operate PX4, you need at least %2 channels.").arg(_chanCount).arg(_chanMinimum));
            }
            return;
        }
        _startCalibration();
...
void RadioComponentController::_startCalibration(void)
{
    ...
    // lysd // _nextButton->setProperty("text", tr("Next"));
...
void RadioComponentController::_stopCalibration(void)
{
    ...
    // lysd // if(_statusText)   _statusText->setProperty("text", "");
    // lysd // if(_nextButton)   _nextButton->setProperty("text", tr("Calibrate"));
    // lysd // if(_nextButton)   _nextButton->setEnabled(true);
    // lysd // if(_cancelButton) _cancelButton->setEnabled(false);
    // lysd // if(_skipButton)   _skipButton->setEnabled(false);
...
void RadioComponentController::_rcCalSave(void)
{
    ...
    // lysd // if(_statusText) _statusText->setProperty(
    // lysd //     "text",
    // lysd //     tr("The current calibration settings are now displayed for each channel on screen.\n\n"
    // lysd //     "Click the Next button to upload calibration to board. Click Cancel if you don't want to save these values."));

    // lysd // if(_nextButton)     _nextButton->setEnabled(true);
    // lysd // if(_skipButton)     _skipButton->setEnabled(false);
    // lysd // if(_cancelButton)   _cancelButton->setEnabled(true);
