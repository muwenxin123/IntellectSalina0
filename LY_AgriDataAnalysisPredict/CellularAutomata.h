...
public:
	...
	void setGridSize(double grid_w);
	void setInitFireInfo(double lon, double lat, double l, double w);
	void setSpreadTime(int time_sec);
	void setTimeStep(int time_sec);
	bool spreadFire(const QVector<int> &weatherConditionVec, QQueue<QVector<QPointF>>& outQueue);
	bool spreadFire(const QVector<int> &weatherConditionVec, std::vector<std::vector<double>>& outDltVec);
	int spreadLayer();
