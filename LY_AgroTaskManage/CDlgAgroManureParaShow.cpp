...
#include "ui_CDlgAgroManureParaShow.h"
#include "QCString.h"
...
CDlgAgroManureParaShow::CDlgAgroManureParaShow(QWidget *parent) :
	LyDialog(tr2("规则内容"), parent),
	ui(new Ui::CDlgAgroManureParaShow)
{
	...
	ui->label_13->hide();
	ui->spray_name_comBox_2->hide();
	ui->label_8->hide();
	ui->compute_box->hide();
	ui->pushButton_4->hide();
	ui->pushButton_6->hide();

	ui->edit_width_5->setText("50");
	ui->edit_width_6->setText("4.008");
...
CDlgAgroManureParaShow::~CDlgAgroManureParaShow()
{
    delete ui;
}

void CDlgAgroManureParaShow::SetStringName(const QString& strTmp)
{
	ui->label_2->setText(strTmp);
}

void CDlgAgroManureParaShow::SetStringNYCS(const QString& strTmp)
{
	ui->textEdit->setText(strTmp);
}

void CDlgAgroManureParaShow::InitUI()
{
	QStringList headerlabels;
	headerlabels << tr2("序号") << tr2("经度") << tr2("纬度") << tr2("高度") << tr2("速度") << tr2("幅宽") << tr2("设备")  << tr2("播撒开关") << tr2("转速") << tr2("距离");
	...
	headerlabels.clear();
	headerlabels << tr2("") << tr2("杀虫剂") << tr2("杀菌剂") << tr2("调节剂") << tr2("生长肥") << tr2("助剂") << tr2("总量")  ;
	...
	ui->drone_box->setCurrentIndex(1);

	ui->route_diration_box->clear();
	QStringList strFXList;
	strFXList << tr2("左上") << tr2("左下") << tr2("右上") << tr2("右下");
	ui->route_diration_box->addItems(strFXList);
	ui->route_diration_box->setCurrentIndex(0);
}

void CDlgAgroManureParaShow::InitCtrl()
{
	ui->label_2->setText("");
	ui->textEdit->setText("");
}

void CDlgAgroManureParaShow::SetAgriFieldInfo(const AgriFieldInfo& stInfo)
{
	m_FieldInfo = stInfo;
...
void CDlgAgroManureParaShow::SetDrugRatioVec(const CDrugRatioVec& RatioVec)
{
	...
	int index = 1;
	ui->tableWidget->setRowCount(2);
	QTableWidgetItem *item11 = new QTableWidgetItem(tr2("名称"));
	...
	QTableWidgetItem *item21 = new QTableWidgetItem(tr2("用量(kg/亩)"));
	...
	{
		ui->tableWidget->item(0, nNo)->setText(itr.stDrugName);
		ui->tableWidget->item(1, nNo)->setText(QString::number(itr.nRaito));
		nRatioCount += itr.nRaito;
		nNo++;
	}
	item17->setText(tr2("总量"));
...
void CDlgAgroManureParaShow::draw2DPathLine(QPainter &painter, IProj *pProj)
{
...
void CDlgAgroManureParaShow::localWeaponNameBindToId()
{
...
void CDlgAgroManureParaShow::InitWeaponCombobox()
{
	ui->drone_box->clear();
...
void CDlgAgroManureParaShow::AddLineToServer(const std::vector<Poi>& LinePoints)
{
...
void CDlgAgroManureParaShow::addTaskSummary()
{
...
void CDlgAgroManureParaShow::SetWorkTime(const long long & llTime1, const long long & llTime2)
{
	m_StartTime = llTime1;
	m_EndTIme = llTime2;
...
void CDlgAgroManureParaShow::UpdateLineListDate()
{
	ui->tableWidget_2->clearContents();
	ui->tableWidget_2->setRowCount(m_flyPoints.size());
	int index = 1;
	int row = 0;
	double dAlt = ui->spinBox_3->value();
	double dSpeed = ui->spinBox_4->value();
	int nFK = ui->spinBox->value();
	QGeoCoordinate qPointStart, qPointEnd;
	double dDis(0),dDisSum(0);
	for (const auto &poi : m_flyPoints)
	{
		qPointEnd.setLongitude(poi.lon);
		qPointEnd.setLatitude(poi.lat);
		if (row == 0)
		{
			qPointStart = qPointEnd;
		}
		dDis = qPointStart.distanceTo(qPointEnd);
		dDisSum += dDis;
		qPointStart = qPointEnd;

		QTableWidgetItem *item1 = new QTableWidgetItem(QString::number(index));
		QTableWidgetItem *item2 = new QTableWidgetItem(QString::number(poi.lon, 'f', 7));
		QTableWidgetItem *item3 = new QTableWidgetItem(QString::number(poi.lat, 'f', 7));
		QTableWidgetItem *item4 = new QTableWidgetItem(QString::number(dAlt, 'f', 2));
		QTableWidgetItem *item5 = new QTableWidgetItem(QString::number(dSpeed, 'f', 2));
		QTableWidgetItem *item6 = new QTableWidgetItem(QString::number(nFK));
		QTableWidgetItem *item7 = new QTableWidgetItem(ui->drone_box->currentText());
		QTableWidgetItem *item8 = new QTableWidgetItem(tr2("开"));
		QTableWidgetItem *item9 = new QTableWidgetItem(tr2("30%"));
		QTableWidgetItem *item10 = new QTableWidgetItem(QString::number(dDis, 'f', 2));
	...
	ui->flight_point->setText(QString::number(m_flyPoints.size()));
	ui->flight_distance->setText(QString::number(dDisSum, 'f', 2));

}

void CDlgAgroManureParaShow::GenerateLine()
{
	m_flyPoints.clear();
	if (LY_AgroTaskManage::Get().getPCDlgScanParaShow() != nullptr)
	{
		std::vector<QPointF> sPointVec = LY_AgroTaskManage::Get().getPCDlgScanParaShow()->getPathPlanPoiVec(m_FieldInfo.area_id, ui->spinBox->value(), 1);
		for (auto itr : sPointVec)
		{
			Poi stPoi(itr.rx(), itr.ry());
			m_flyPoints.push_back(stPoi);
		}
	}
	...
	m_isGeneratingCourse = true;
}

void CDlgAgroManureParaShow::hideEvent(QHideEvent *event)
{
	InitCtrl();
	m_spayMapshow = false;
...
void CDlgAgroManureParaShow::GenerateSprayPrescriptionDiagram()
{
...
void CDlgAgroManureParaShow::on_pushButton_clicked()
{
	InitCtrl();
	this->hide();
}

void CDlgAgroManureParaShow::on_pushButton_4_clicked()
{
	m_spayMapshow = !m_spayMapshow;
	IAgroTaskManagerActivator::Get().GetPIAgriSprayPrescriptionService()->showSprayDiagramData(m_spayMapshow);
}

void CDlgAgroManureParaShow::on_pushButton_3_clicked()
{
	m_isGeneratingCourse = !m_isGeneratingCourse;
}

void CDlgAgroManureParaShow::on_pushButton_6_clicked()
{
	m_isInstructionStatus = !m_isInstructionStatus;
}

void CDlgAgroManureParaShow::on_pushButton_5_clicked()
{
	GenerateLine();
}

void CDlgAgroManureParaShow::on_pushButton_2_clicked()
{
	LY_AgroTaskManageInterface::Get().SendMsgToAll(IInterface::MSG_Command, 50009, nullptr);
}

void CDlgAgroManureParaShow::on_spray_name_comBox_2_currentIndexChanged(const int &arg1)
{
