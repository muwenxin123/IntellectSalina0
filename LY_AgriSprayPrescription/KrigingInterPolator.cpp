...
void HeatMap3D::createGridGeometry(std::vector<std::vector<double>>& grid, double minLon, double minLat, double spacing,
	std::vector<osgEarth::Features::FeatureList>& featureLists, float angle)
{
	...
	for (int x = 0; x < grid.size(); x++)
	{
		for (int y = 0; y < grid[0].size(); y++)
		{
			...
			//if (angle == 0)
			//{
			//	v1 = origin + osg::Vec3d(y * spacing, x * spacing, 0);
			//	/*if (!(polygon->contains2D(v1.x(), v1.y())))
			//	{
			//		continue;
			//	}*/
			//	v2 = v1 + osg::Vec3d(0, spacing, 0);
			//	v3 = v1 + osg::Vec3d(spacing, spacing, 0);
			//	v4 = v1 + osg::Vec3d(spacing, 0, 0);
			//}
			//else
			{
				...
				bearingRad = atan(((y * 1.0) + 0.5) / ((x * 1.0) + 0.5)) + angle;
				//disSpacing 单位为m，是网格的大小
				double Tempdistance1 = sqrt(((x * 1.0) + 0.5) * ((x * 1.0) + 0.5) + ((y * 1.0) + 0.5) * ((y * 1.0) + 0.5)) * disSpacing;
				//destination()函数内的单位都为弧度制，需要从度°转为弧度
				osgEarth::GeoMath::destination(osg::DegreesToRadians(minLat), osg::DegreesToRadians(minLon),
				bearingRad, Tempdistance, CenLatR, CenLonR);
				//CenLatR, CenLonR为计算出来的目标点的经纬度（弧度制),用时需使用函数将其转为度°
				}
				*/
				...
				v3 = v1 + osg::Vec3d(spacing, spacing, 0);
				v4 = v1 + osg::Vec3d(spacing, 0, 0);
				v1.x() = (minLon + (v1.x() - minLon) * cos(-angle) - (v1.y() - minLat) * sin(-angle));
				v1.y() = minLat + (v1.x() - minLon) * sin(-angle) + (v1.y() - minLat) * cos(-angle);*/
				/*if (!(polygon->contains2D(v1.x(), v1.y())))
				{
				continue;
				}*/
				/*v2.x() = (minLon + (v2.x() - minLon) * cos(-angle) - (v2.y() - minLat) * sin(-angle));
				v2.y() = minLat + (v2.x() - minLon) * sin(-angle) + (v2.y() - minLat) * cos(-angle);

				v3.x() = (minLon + (v3.x() - minLon) * cos(-angle) - (v3.y() - minLat) * sin(-angle));
				v3.y() = minLat + (v3.x() - minLon) * sin(-angle) + (v3.y() - minLat) * cos(-angle);

				v4.x() = (minLon + (v4.x() - minLon) * cos(-angle) - (v4.y() - minLat) * sin(-angle));
				v4.y() = minLat + (v4.x() - minLon) * sin(-angle) + (v4.y() - minLat) * cos(-angle);*/

				//osg::Vec3d v1 = origin + osg::Vec3d( -(x * spacing * sin(-angle)) + (y * spacing * cos(-angle)), (x * spacing * cos(-angle)) + ( y * spacing * sin(-angle)), 0);
				/* v1 = origin + osg::Vec3d(-(x * spacing * sin(-angle)) + (y * spacing * cos(-angle)),
				(x* spacing * cos(-angle)) + (y * spacing * sin(-angle)), 0);*/
				/*if (!(polygon->contains2D(v1.x(), v1.y())))
				{
				continue;
				}
				*/
				/*osg::Vec3d v2 = v1 + osg::Vec3d(spacing * cos(-angle), - spacing *  sin(-angle), 0);
				osg::Vec3d v3 = v1 + osg::Vec3d(spacing *  (cos(-angle)  + sin(-angle)), spacing * (cos(-angle) - sin(-angle)), 0);
				osg::Vec3d v4 = v1 + osg::Vec3d(spacing *  sin(-angle), spacing * cos(-angle), 0);*/
				/* v2 = v1 + osg::Vec3d( -spacing *  sin(-angle), spacing * cos(-angle), 0);
				v3 = v1 + osg::Vec3d(spacing * (cos(-angle) - sin(-angle)), spacing *  (cos(-angle) + sin(-angle)), 0);
				v4 = v1 + osg::Vec3d(spacing * cos(-angle), spacing *  sin(-angle), 0);*/
			}
			...
			/*double cenLon = (2.0 * y + 1.0) * spacing / 2.0 + minLon;
			double cenLat = (2.0 * x + 1.0) * spacing / 2.0 + minLat;*/
			//double CenLonR, CenLatR;
