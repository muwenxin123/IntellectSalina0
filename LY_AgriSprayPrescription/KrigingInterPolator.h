...
public:
	...
	std::vector<std::pair<double, AgriSprayPrescriptionDecl::AirInstructionPoi>>airInstructionPoints;
...
private:
	...
	void HeatMap3D::createGridGeometry(std::vector<std::vector<double>>& grid, double minLon, double minLat, double spacing
		, std::vector<osgEarth::Features::FeatureList>& featureLists, float angle = 0.0);         // 新增：旋转中心纬度
