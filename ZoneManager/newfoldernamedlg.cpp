...
NewFolderNameDlg::NewFolderNameDlg(const QString &WndName, const QString &FileName, int maxname, QWidget *pParent, bool isModal): LyDialog(tr2("新建文件夹"), pParent), ui(new Ui::NewFolderNameDialog())
{
...
void NewFolderNameDlg::OnOK()
{
    ...
    if (ZoneManager::Get().CheckNameStr(m_FileName, m_maxnamelen) == false)
    {
        return;
    }
