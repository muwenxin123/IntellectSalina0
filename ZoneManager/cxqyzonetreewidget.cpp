...
void CXQYZoneTreeWidget::OnItemChangedSlot(QTreeWidgetItem *item, int col)
{
    ...
    if (m_CurInd != -1)
    {
        ...
        if (m_pQYZoneDlg)
        {
            if (m_ZoneArray[m_CurInd].show[0] && !m_pQYZoneDlg->m_bCreateFlag && m_ZoneArray[m_CurInd].data.n != 0)
            {
                m_pQYZoneDlg->DingWeiqy(m_ZoneArray[m_CurInd]);
            }
        }
...
void CXQYZoneTreeWidget::OnTreeSelectionSlot()
{
    ...
    {
        m_pQYZoneDlg->WriteKyList(m_ZoneArray[m_CurInd]);
    }
