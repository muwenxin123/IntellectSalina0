...
{
    // lysd for (const auto[key, value] : m_pendingUnsubscriptions.asKeyValueRange()) {
    // lysd     if (value == sub)
    // lysd         return true;
    // lysd }
    ...
    QHash<quint16, QMqttSubscription *>::const_iterator iter = m_pendingUnsubscriptions.constBegin();
    for (; iter != m_pendingUnsubscriptions.constEnd(); iter++) {
        if (iter.value() == sub)
            return true;
    }
...
void QMqttConnection::connectTransport(QMqttClient::TransportType transportType, const std::shared_ptr<QIODevice> &transport)
{
    ...
    switch (m_transportType) {
        ...
        case QMqttClient::TransportType::AbstractSocket:
            ...
            // lysd QObject::connect(qobject_cast<QAbstractSocket *>(m_transport.get()), &QAbstractSocket::errorOccurred, this, &QMqttConnection::transportError);
#if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
            ...
            QObject::connect(qobject_cast<QAbstractSocket *>(m_transport.get()), QOverload<QAbstractSocket::SocketError>::of(&QAbstractSocket::error), this, &QMqttConnection::transportError);
        ...
        case QMqttClient::TransportType::SecureSocket:
            ...
            // lysd QObject::connect(qobject_cast<QSslSocket *>(m_transport.get()), &QAbstractSocket::errorOccurred, this, &QMqttConnection::transportError);
#if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
            ...
            QObject::connect(qobject_cast<QSslSocket *>(m_transport.get()), QOverload<QAbstractSocket::SocketError>::of(&QAbstractSocket::error), this, &QMqttConnection::transportError);
...
bool QMqttConnection::ensureTransport(bool createSecureIfNeeded)
{
    ...
    if (createSecureIfNeeded) {
        ...
        // lysd QObject::connect(socket.get(), &QAbstractSocket::errorOccurred, this, &QMqttConnection::transportError);
#if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
        ...
        QObject::connect(socket.get(), QOverload<QAbstractSocket::SocketError>::of(&QAbstractSocket::error), this, &QMqttConnection::transportError);
...
void QMqttConnection::finalize_connack()
{
    ...
    if (m_clientPrivate->m_protocolVersion == QMqttClient::MQTT_5_0) {
        ...
        // lysd m_receiveAliases.resize(m_clientPrivate->m_serverConnectionProperties.maximumTopicAlias());
        // lysd m_publishAliases.resize(m_clientPrivate->m_connectionProperties.maximumTopicAlias());
#if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
...
void QMqttConnection::finalize_publish()
{
    ...
    // lysd for (const auto[key, value] : m_activeSubscriptions.asKeyValueRange()) {
    // lysd    if (key.match(topic))
    // lysd        subscribers.append(value);
    // lysd }
    ...
    QMultiHash<QMqttTopicFilter, QMqttSubscription *>::const_iterator iter = m_activeSubscriptions.constBegin();
    for (; iter != m_activeSubscriptions.constEnd(); iter++) {
        if (iter.key().match(topic))
            subscribers.append(iter.value());
    }
