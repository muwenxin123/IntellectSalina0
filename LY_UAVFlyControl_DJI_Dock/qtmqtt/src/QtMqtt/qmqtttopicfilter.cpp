...
bool QMqttTopicFilter::match(const QMqttTopicName &name, MatchOptions matchOptions) const
{
    ...
    if (d->filter.endsWith(QLatin1Char('#'))) {
		...
		// lysd const auto filterLevels = root.split(QLatin1Char('/'));
		// lysd const auto topicLevels = QStringView{ topic }.split(QLatin1Char('/'));
		...
		const auto filterLevels = QString(root.toString()).split(QLatin1Char('/'));
    ...
    if (d->filter.contains(QLatin1Char('+'))) {
        // lysd const auto filterLevels = QStringView{d->filter}.split(QLatin1Char('/'));
		// lysd const auto topicLevels = QStringView{topic}.split(QLatin1Char('/'));
#if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
...
{
	// lysd return qHash(filter.d->filter, seed);
#if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
	...
	return qHash(QUrl(filter.d->filter), seed);
