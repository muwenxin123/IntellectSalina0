...
{
    // lysd return d->name.split(QLatin1Char('/'), Qt::KeepEmptyParts);
	...
	return d->name.split(QLatin1Char('/'), QString::SplitBehavior::KeepEmptyParts);
...
{
	// lysd return qHash(name.d->name, seed);
	...
	return qHash(QUrl(name.d->name), (uint)seed);
