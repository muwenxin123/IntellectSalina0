...
{
	ui->comboBox_UAVFlyManage_MavlinkTaskManage_Task->setCurrentText(strTask);
}
...
void UAVFlyManage_MavlinkTaskManageDialog::InitWeaponInfoList()
{
	...
	auto lastLine = *(allLine.rbegin());

	if (lastLine.second.LinePointVec.size() < 2)
	{
		return;
	}

	//添加起飞点和返航点
	CAirTrafficLinePoint stTakeoff = lastLine.second.LinePointVec[0];
	CAirTrafficLinePoint stReturn = lastLine.second.LinePointVec[lastLine.second.LinePointVec.size() - 1];
	lastLine.second.LinePointVec.insert(lastLine.second.LinePointVec.begin(), stTakeoff);
	lastLine.second.LinePointVec.push_back(stReturn);

	int nUAVID = QString::fromLocal8Bit(lastLine.second.strUAVID).toInt();
	...
	for (int i = 0; i < vehiclesCount; i++)
	{
		...
		for (const auto &poi : lastLine.second.LinePointVec) {
		...
		strLineName += ":";
		strLineName += QString::fromLocal8Bit(lastLine.second.strName);
