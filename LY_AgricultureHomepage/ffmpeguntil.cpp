...
FFmpegUntil::FFmpegUntil(QObject *parent) : QObject(parent)
{
	MyFFmpegInfo();

	startPlay();
	connect(&timer, &QTimer::timeout, this, &FFmpegUntil::timerTimeout);
	timer.start(30000); // 定时器每隔1000毫秒触发一次timeout信号
}

FFmpegUntil::~FFmpegUntil()
{
	//MyFFmpegDestroy();
}
void FFmpegUntil::timerTimeout()
{

	startPlay();

}
void FFmpegUntil::MyFFmpegInfo()
{
	qDebug("------------------------------------------------------------------");
...
void FFmpegUntil::MyFFmpegSetUrl(QString rtspUrl)
{
	
	this->m_rtspUrl = rtspUrl;
}

void FFmpegUntil::MyFFmpegDestroy()
{
	m_stopped = true;
	...
	AVFormatContext* old = m_AVFormatContext.exchange(nullptr);
	if (old) {
...
int FFmpegUntil::MyFFmpegInit()
{
	...
	if ((m_AVFormatContext = avformat_alloc_context() )== nullptr)
	{
	...
	if (!isLocalVideo)
	{
		...
		ret = avformat_open_input(&ctx, pRtspUrl, NULL, &options);
		if (ret == 0)
		{
			m_AVFormatContext.store(ctx);
		}
	}
	else
	{
		ret = avformat_open_input(&ctx, m_rtspUrl.toStdString().c_str(), NULL, NULL);
   ...
   /* qDebug() << "file iformat: " << m_AVFormatContext->iformat->name;
    qDebug() << "file duration: " << m_AVFormatContext->duration << " microseconds";
    qDebug() << "file bit_rate: " << m_AVFormatContext->bit_rate;
*/
	...
	m_AVCodecContext = ctx->streams[m_videoIndex]->codec;
	...
	if (m_AVCodecContext->pix_fmt == -1 || m_AVCodecContext->width == 0 || m_AVCodecContext->height == 0)
	{
		return -1;
	}
...
int FFmpegUntil::MyFFmpepReadFrame()
{
	int ret = -1;
	int getPicture = 0;
	...
	AVFormatContext* ctx = m_AVFormatContext.load();
	if (!ctx) return -1;

	std::lock_guard<std::mutex> lock(m_operationMutex);
	//if (m_AVFormatContext == nullptr || m_AVFormatContext == NULL)
	//{
	//	return -1;
	//}
	ret = av_read_frame(ctx, m_AVPacket);
	if (ret < 0)
	{
	...
	//qDebug() << "star code: " << startTime.msecsTo(endTime) << "ms";
	if (ret < 0)
	{
...
void FFmpegUntil::closeEvent(QCloseEvent *e)
{
	qDebug() << "=================";
}
void FFmpegUntil::startPlay()
{
	// qDebug() << "start Plat" << QThread::currentThread() << "start play";
	if (this->m_rtspUrl == nullptr)
	{
		stopPlay();
		return;
	}
	this->MyFFmpegSetUrl(this->m_rtspUrl);

	int ret = this->MyFFmpegInit();
	if (ret != 0)
	{
		stopPlay();
		return;
	...
	qDebug() << "======startPlay===***************:" << QThread::currentThread();
	//string cfg_file = "../3rdParty/bin/yolo/nongye.cfg";
	//string weight_file = "../3rdParty/bin/yolo/nongye.weights";
	//// 创建检测对象
	...
	while (1) {
		...
		if (m_rtspUrl == nullptr) {
			break;
		}

		if (this->MyFFmpepReadFrame() < 0) {
...
void FFmpegUntil::stopPlay()
{
	m_stopped = true;
	//MyFFmpegDestroy();
}

void FFmpegUntil::doDealingMessage()
{
	startPlay();
}
