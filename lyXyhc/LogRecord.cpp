...
#define MAX_LOGRECORD_FILE_SIZE            536800000
#define MAX_LOGRECORD_FILE_ROW             1048000
#define MAX_LOGRECORD_FILE_XYGSSERVER_SIZE 536800000
#define MAX_LOGRECORD_FILE_XYGSSERVER_ROW  1048000
...
bool CLogRecord::open() {
    ...
    _errno = fopen_s(&_logStream, _fileName.c_str(), _fileMode.c_str());
    if (_errno != 0) {
...
bool CLogRecord::open(const std::string& fileName, const std::string fileMode) {
    ...
    _errno = fopen_s(&_logStream, fileName.c_str(), fileMode.c_str());
    if (_errno != 0) {
        _errorStr = ("The file " + _fileName + " was not opened, mode is " + _fileMode + ", errorno is " + std::to_string(_errno) + ".");
        return false;
    }
...
size_t CLogRecordCVS::write(const char* buffer, size_t elementSize) {
    if (buffer != nullptr) {
        ...
        if (result) {
            flush();
            _logRow++;
            _logSize = _logSize + result * elementSize;
        }
