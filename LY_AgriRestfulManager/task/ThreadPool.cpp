...
:taskQueue_(new OETaskQueue<ITask>()), atcCurTotalThrNum_(0), atcWorking_(true) {
}
...
// 添加任务
int OEThreadPool::addTask(std::shared_ptr<ITask> taskptr, bool priority) {
...
std::shared_ptr<ITask> OEThreadPool::isTaskProcessed(size_t nId) {
    return taskQueue_->isTaskProcessed(nId);
}


std::shared_ptr<ITask> OEThreadPool::isExistTaskProcess()
{
	return taskQueue_->isExistTaskProcess();
}
