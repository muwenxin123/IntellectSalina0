...
//
////插入队列 
//void OETaskQueue::put_back(std::shared_ptr<ITask>& task) {
...
//}
//void OETaskQueue::put_front(std::shared_ptr<ITask>& task) {
//	std::unique_lock<std::mutex> lock(mutex_);
//
...
//}
////获取队首（并将任务加到运行任务列表中）
//std::shared_ptr<ITask> OETaskQueue::get(void) {
//
//	std::unique_lock<std::mutex> lock(mutex_);
//
//	if (queue_.empty()) 
...
//	
//
//	std::shared_ptr<ITask> &task = queue_.front();
...
//
//		// std::unordered_map<int64_t, std::shared_ptr<ITask>>::iterator
...
//	std::unique_lock<std::mutex> lock(mutex_);
//	// std::deque<std::shared_ptr<ITask>>::iterator
//	auto it = queue_.begin();
//	for (; it != queue_.end(); ++it) {
//
//		if ((*it)->getID() == nId) 
