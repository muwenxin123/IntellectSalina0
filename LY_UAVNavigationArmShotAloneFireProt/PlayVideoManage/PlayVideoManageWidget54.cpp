  ...
  , m_strFileNameInfrared("")
{
    ...
    QSettings settings(strApplicationDirPath + "/../data/" + "YoloConfig.ini", QSettings::IniFormat);
...
bool PlayVideoManageWidget54::StartVideoManageThread(const CVideoDevice& videoDevice)
{
    ...
    m_VideoDeviceParam.SetVideoDeviceType54(false);
    ...
    if (videoDevice.GetVideoDeviceType() == "rtsp54") {
        ...
        if(m_pRecvVideoThread == nullptr){
            m_pRecvVideoThreadObject = new RecvVideoStreamThread(QString::fromStdString(m_strFileName), m_VideoDeviceParam, nullptr);
...
NetworkWorkerThread::NetworkWorkerThread(const multicast_configure_t &multicast_configure, QObject *parent)
    : QThread(parent)
    , _init(false)
...
void RecvVideoStreamThread::video_doWork()
{
    ...
    if(m_UrlNameDeviceType54){
        ...
        if(url.isValid()){
            ...
            m_MulticastNetworkWorkerThread = new NetworkWorkerThread(multi);
            if(m_MulticastNetworkWorkerThread){
