	...
	, m_video_localaddr("")
{
	...
	settings.beginGroup("VIDEO_LOCALADDR_SETTING");

	if (settings.contains("Video_LocalAddr") && settings.value("Video_LocalAddr").canConvert(QVariant::Type::String))
	{
		const QString &video_LocalAddr = settings.value("Video_LocalAddr", "").toString();
		if (!video_LocalAddr.isEmpty())
		{
			m_video_localaddr = video_LocalAddr.toStdString();
		}
	}

	{
		...
		m_enableCollection = showFlagVideoRealTimePreview;
	}
...
bool PlayVideoManageWidget::StartVideoManageThread(const CVideoDevice &videoDevice)
{
	...
	if (videoDevice.GetVideoDeviceType() == "hk")
	{
		...
		m_RealPlay.SetIdentity(std::atoi(m_VideoDevice.GetVideoComponentID().c_str()));
		m_RealPlay.SetPlayWnd(m_WId);
...
void qnzkna::PlayVideoManage::PlayVideoManageWidget::appendAvframe(AVFrame* avframe)
{
...
bool qnzkna::PlayVideoManage::PlayVideoManageWidget::canCollection()
{
	return m_enableCollection;
}

void qnzkna::PlayVideoManage::PlayVideoManageWidget::paintEvent(QPaintEvent *event)
{
...
static int RecvVideoThreadFun(void *lpParameter)
{
	...
	const bool showFlagVideoRealTimePreview = playVideoManage->canCollection();
	...
	const bool video_localaddrEmpty = playVideoManage->m_video_localaddr.empty();
	...
	strncpy(video_localaddr, playVideoManage->m_video_localaddr.c_str(), 255);
	playVideoManage->UnLock();
	//#endif
...
#endif
	int ctx_width = 0, ctx_height = 0;
	int newInput = 0;
	int got_picture = 0;

	double max_custom_time = 0;
	int capture_image_intervel = 0;
	...
	QString tmpHeader = QString::fromStdString(fileName);
	if (tmpHeader.startsWith("rtsp://"))
	{
		av_dict_set(&opts, "rtsp_transport", "tcp", 0);
	}

	if (!video_localaddrEmpty) {
		av_dict_set(&opts, "localaddr", video_localaddr, 0);
	}
		...
		if (p_packet->stream_index == v_idx)
		{
			...
			do
			{
				...
				if (got_picture > 0)
				{
					...
					if (showFlagVideoRealTimePreview &&nullptr != p_frame_raw)
#else
					if (showFlagVideoRealTimePreview && nullptr != p_frame_raw && capture_image_intervel % 4 == 0)
#endif
					{

						auto avFrame = p_frame_raw;
						if (nullptr != avFrame && avFrame->data[0])
						{
							AVFrame* p_frmav_tmp = av_frame_clone(avFrame);
							if (nullptr != p_frmav_tmp)
							{
								playVideoManage->appendAvframe(p_frmav_tmp);
							}
						}
					}
		}
