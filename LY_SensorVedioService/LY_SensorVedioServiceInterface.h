#ifndef _LY_SENSORVEDIOSERVICE_H
#define _LY_SENSORVEDIOSERVICE_H

#include <IInterface.h>

class LY_SensorVedioServiceInterface : public IInterface
{
public:
    //inline static LY_SensorVedioServiceInterface& Get(){static LY_SensorVedioServiceInterface gi; return gi;};
    //写成以上形式在Windows操作系统上使用VC2008编译、运行没有问题，使用QT5.5.1自带的MinGw4.9.2编译、运行也正常。
    //Ubuntu14.04操作系统上使用gcc4.8.2编译时，不能同时加载多个LY_SensorVedioServiceInterface::Get()实例对象，或者改gi的名称不重复，
    //或者改为如下形式，并且必须将LY_SensorVedioServiceInterface::Get()的实现函数放到cpp文件中，对象gi的名称即可以重复，在使用QLibrary加载不同的动态库中对象gi的地址是不同的，可以达到预期目的。
    static LY_SensorVedioServiceInterface &Get();
...
#endif // _LY_SENSORVEDIOSERVICE_H
