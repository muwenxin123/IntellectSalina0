...
drawAble_Circle::drawAble_Circle()
{
}
...
void drawAble_Circle::OnEarthMessage(int wParam, void * lParam)
{
	...
	case IEarthEventHandler::Earth_LDOUBLECLICK:
	{
		...
		if (featureNode == nullptr)
		{
			...
			featureNode = new osgEarth::Annotation::CircleNode(mapNode,
	...
	{
		if (isTrackMoving == false || points.empty())
			return;
		float radius = osgEarth::GeoMath::distance(points[0], pos, mapNode->getMapSRS()->getGeodeticSRS());
		dynamic_cast<osgEarth::Annotation::CircleNode*>(featureNode)->setRadius(radius);
		LY_ZoneManagerServiveInterface::Get().SendMsgToAll(IInterface::MSG_Command, 1001003, &radius);
